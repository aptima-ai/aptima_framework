...
class data_t : public msg_t {
 ...
 public:
  static std::unique_ptr<data_t> create(const char *data_name,
    ...
    auto *c_data = axis_data_create(
        data_name, err != nullptr ? err->get_c_error() : nullptr);

    return std::make_unique<data_t>(c_data, ctor_passkey_t());
  }

  explicit data_t(axis_shared_ptr_t *data, ctor_passkey_t /*unused*/)
      : msg_t(data) {}

  ~data_t() override = default;

  bool alloc_buf(size_t size, error_t *err = nullptr) {
    return axis_data_alloc_buf(c_msg, size) != nullptr;
  }

  buf_t lock_buf(error_t *err = nullptr) {
    if (!axis_msg_add_locked_res_buf(
            c_msg, axis_data_peek_buf(c_msg)->data,
            err != nullptr ? err->get_c_error() : nullptr)) {
      return {};
    }

    buf_t result{axis_data_peek_buf(c_msg)->data,
                 axis_data_peek_buf(c_msg)->size};
  ...
  buf_t get_buf(error_t *err = nullptr) const {
    size_t data_size = axis_data_peek_buf(c_msg)->size;

    buf_t buf{data_size};
    if (data_size != 0) {
      memcpy(buf.data(), axis_data_peek_buf(c_msg)->data, data_size);
    }
    return buf;
  }
  ...
  explicit data_t(axis_shared_ptr_t *data) : msg_t(data) {}
  // @}
};
