 ...
 * 'axis_connection_on_msgs()') _before_ the connection migration is totally
 * completed.
 *
 ...
 *      /// migration.
 *      axis_runloop_post_task(engine_thread, protocol::on_migrated);
 *   }
 *
 * - protocol::on_migrated() {
 ...
 *      axis_runloop_post_task(app_thread, connection::clean);
 *   }
 *
 * - connection::clean() {
 *      /// Calling in the app thread.
 *
 *      axis_app_del_orphaned_connection(connection);
 *      protocol::clean();
 *   }
 *
 * - protocol::clean() {
 ...
 *      axis_runloop_post_task(engine_thread, protocol::on_cleaned);
 *   }
 *
 * - protocol::on_cleaned() {
 *       /// Calling in the engine thread.
 ...
 *   }
 */
axis_RUNTIME_PRIVATE_API void axis_connection_migrate(axis_connection_t *self,
 ...
 */
axis_RUNTIME_PRIVATE_API void axis_connection_upgrade_migration_state_to_done(
    axis_connection_t *self, axis_engine_t *engine);
...
axis_RUNTIME_PRIVATE_API axis_CONNECTION_MIGRATION_STATE
axis_connection_get_migration_state(axis_connection_t *self);

axis_RUNTIME_PRIVATE_API void axis_connection_set_migration_state(
