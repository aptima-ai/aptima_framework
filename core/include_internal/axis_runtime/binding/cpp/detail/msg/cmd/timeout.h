...
class cmd_timeout_t : public cmd_t {
 private:
  ...
  static std::unique_ptr<cmd_timeout_t> create(axis_shared_ptr_t *cmd,
                                               error_t *err = nullptr) {
    return std::make_unique<cmd_timeout_t>(cmd, ctor_passkey_t());
  }

  explicit cmd_timeout_t(axis_shared_ptr_t *cmd) : cmd_t(cmd) {}

 public:
  explicit cmd_timeout_t(axis_shared_ptr_t *cmd, ctor_passkey_t /*unused*/)
      : cmd_t(cmd) {}
  ~cmd_timeout_t() override = default;

  uint32_t get_timer_id(error_t *err = nullptr) const {
    return axis_cmd_timeout_get_timer_id(c_msg);
  }
