...
bool axis_base64_to_string(axis_string_t *result, axis_buf_t *buf) {
  axis_ASSERT(result && axis_string_check_integrity(result), "Invalid argument.");
  axis_ASSERT(buf && axis_buf_check_integrity(buf), "Invalid argument.");
  ...
  if (l > 0) {
    x <<= 6 - l;
    axis_string_append_formatted(result, "%c", base64_chars[x & 0x3f]);
  }

  for (; axis_string_len(result) % 4;) {
    axis_string_append_formatted(result, "%c", '=');
  }
...
bool axis_base64_from_string(axis_string_t *str, axis_buf_t *result) {
  axis_ASSERT(str && axis_string_check_integrity(str), "Invalid argument.");
  axis_ASSERT(result && axis_buf_check_integrity(result), "Invalid argument.");
  ...
  const char *src_ptr = axis_string_get_raw_str(str);
  size_t src_size = axis_string_len(str);
  ...
  base64.x = 0;
  uint8_t *base64_decoded = (uint8_t *)axis_MALLOC(src_size);
  axis_ASSERT(base64_decoded, "Failed to allocate memory.");
