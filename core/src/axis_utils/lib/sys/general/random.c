...
int axis_random_string(char *buf, size_t size) {
  if (!buf || size <= 1) {
    return -1;
  }

  int r = axis_random(buf, size - 1);
  if (r < 0) {
    return r;
  }
...
int axis_random_hex_string(char *buf, size_t size) {
  ...
  int r = axis_random(ran_buf, len / 2);
  if (r < 0) {
    goto error;
  }
...
int axis_random_base64_string(char *buf, size_t size) {
  (void)buf;
  (void)size;
  return -1;
}

int axis_random_int(int start, int end) {
  uint32_t result = 0;
  int ret = -1;
  ...
  ret = axis_random(&result, sizeof(result));
