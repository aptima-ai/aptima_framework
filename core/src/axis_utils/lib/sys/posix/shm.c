...
void *axis_shm_map(const char *name, size_t size) {
  ...
  if (open_existing) {
    ...
    int rc = ftruncate(fd, size + sizeof(axis_atomic_t));
  ...
  addr = mmap(NULL, size + sizeof(axis_atomic_t), PROT_READ | PROT_WRITE,
              MAP_SHARED, fd, 0);
  if (addr == MAP_FAILED) {
    addr = NULL;
    goto done;
  ...
  if (!open_existing) {
    cookie = (axis_atomic_t *)addr;
    axis_atomic_store(cookie, size);
  }

  addr = (char *)addr + sizeof(axis_atomic_t);
