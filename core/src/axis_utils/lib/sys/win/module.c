...
void *axis_module_load(const axis_string_t *name, int as_local) {
  (void)as_local;
  if (!name || axis_string_is_empty(name)) {
    return NULL;
  }
  ...
  return (void *)LoadLibraryExA(
      axis_string_get_raw_str(name), NULL,
...
void *axis_module_get_symbol(void *handle, const char *symbol_name) {
  ...
  if (!symbol_name) {
    axis_LOGE("Invalid argument: symbol name is null or empty.");
    return NULL;
  }
...
#if 0
    axis_LOGE("Failed to find symbol %s: %s", symbol_name,
