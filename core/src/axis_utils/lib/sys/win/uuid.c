...
void axis_uuid4_seed(axis_uuid4_state_t *seed) {
  static uint64_t state0 = 0;

  LARGE_INTEGER time;
  BOOL ok = QueryPerformanceCounter(&time);
  axis_ASSERT(ok, "Should not happen.");
  ...
  *seed = *seed * 6364136223846793005U +
          ((uint64_t)(axis_uuid4_mix(axis_uuid4_hash(pid), axis_uuid4_hash(tid)))
           << 32);
  *seed = *seed * 6364136223846793005U + (uintptr_t)GetCurrentProcessId;
  *seed = *seed * 6364136223846793005U + (uintptr_t)axis_uuid4_gen_from_seed;
}
