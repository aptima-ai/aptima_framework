...
static int macho_add(axis_backtrace_t *self, const char *filename,
                     int descriptor, off_t offset,
                     const unsigned char *match_uuid, uintptr_t base_address,
                     int skip_symtab, axis_backtrace_error_func_t error_cb,
                     void *data,
                     axis_backtrace_get_file_line_func_t *fileline_fn,
                     int *found_sym);

// A dummy callback function used when we can't find any debug info.
static int macho_nodebug(axis_backtrace_t *self, uintptr_t pc,
...
static void macho_nosyms(axis_backtrace_t *self, uintptr_t addr,
 ...
 */
static int macho_add_dwarf_section(axis_backtrace_t *self, int descriptor,
...
static int macho_add_dwarf_segment(axis_backtrace_t *self, int descriptor,
...
static int macho_add_symtab(axis_backtrace_t *self_, int descriptor,
  ...
  size_t symsize;
  axis_mmap_t sym_view;
  int sym_view_valid;
  axis_mmap_t str_view;
  int str_view_valid;
  size_t ndefs;
  size_t symtaboff;
  unsigned int i;
  ...
  if (!axis_mmap_init(&sym_view, descriptor, symoff, nsyms * symsize)) {
    goto fail;
  }

  sym_view_valid = 1;

  if (!axis_mmap_init(&str_view, descriptor, stroff, strsize)) {
    return 0;
  }
  ...
  macho_symbols =
      (struct macho_symbol *)axis_malloc_without_backtrace(macho_symbol_size);
  ...
  sdata =
      (struct macho_syminfo_data *)axis_malloc_without_backtrace(sizeof *sdata);
  axis_ASSERT(sdata, "Failed to allocate memory.");
  if (sdata == NULL) {
    goto fail;
  }
  ...
  while (1) {
    ...
    while (1) {
      struct macho_syminfo_data *p = axis_atomic_ptr_load((void *)pp);
      if (p == NULL) {
        break;
      }
...
fail:
  if (macho_symbols != NULL) {
    axis_free_without_backtrace(macho_symbols);
  }
...
static void macho_syminfo(axis_backtrace_t *self_, uintptr_t addr,
                          axis_backtrace_dump_syminfo_func_t dump_syminfo_cb,
                          axis_backtrace_error_func_t error_cb, void *data) {
  axis_backtrace_posix_t *self = (axis_backtrace_posix_t *)self_;
  assert(self && "Invalid argument.");
  ...
  while (1) {
    sdata = axis_atomic_ptr_load((void *)pp);
    if (sdata == NULL) {
      break;
    }
...
static int macho_add_fat(axis_backtrace_t *self, const char *filename,
  ...
  if (!axis_mmap_init(&arch_view, descriptor, offset, nfat_arch * arch_size)) {
    goto fail;
  }
...
static int macho_add_dsym(axis_backtrace_t *self, const char *filename,
  ...
  if (p == NULL) {
    ...
    diralc = axis_malloc_without_backtrace(dirnamelen + 1);
    if (diralc == NULL) {
      goto fail;
    }
  ...
  dsym = axis_malloc_without_backtrace(dsymlen);
  axis_ASSERT(dsym, "Failed to allocate memory.");
  if (dsym == NULL) {
    goto fail;
  }
  ...
  *ps = '\0';
  ...
  if (diralc != NULL) {
    axis_free_without_backtrace(diralc);
    diralc = NULL;
  }

  int fd = axis_file_open(dsym, &does_not_exist);
  if (fd < 0) {
    /* The file does not exist, so we can't read the debug info.
       Just return success.  */
    axis_free_without_backtrace(dsym);
    return 1;
  }
...
static int macho_add(axis_backtrace_t *self, const char *filename,
                     ...
                     axis_backtrace_get_file_line_func_t *fileline_fn,
                     int *found_sym) {
  axis_mmap_t header_view;
  struct macho_header_32 header;
  off_t hdroffset;
  int is_64;
  axis_mmap_t cmds_view;
  int cmds_view_valid;
  struct dwarf_sections dwarf_sections;
  int have_dwarf;
  ...
  *found_sym = 0;
  ...
  if (!axis_mmap_init(&header_view, descriptor, offset,
  ...
  if (!axis_mmap_init(&cmds_view, descriptor, hdroffset, header.sizeofcmds)) {
    goto fail;
  }
...
int axis_backtrace_init_posix(
  ...
  int found_sym = 0;
  axis_backtrace_get_file_line_func_t macho_fileline_fn = macho_nodebug;

  uint32_t c = _dyld_image_count();
  for (uint32_t i = 0; i < c; ++i) {
    uintptr_t base_address;
    int d;
    axis_backtrace_get_file_line_func_t mff;
    int mfs;
    ...
    if (strcmp(name, filename) == 0 && !closed_descriptor) {
      ...
      d = axis_file_open(name, &does_not_exist);
      if (d < 0) {
        continue;
      }
    }
  ...
  if (found_sym) {
    axis_atomic_ptr_store((void *)&posix_self->get_syminfo, macho_syminfo);
  } else {
    (void)__sync_bool_compare_and_swap(&posix_self->get_syminfo, NULL,
                                       macho_nosyms);
  }

  *get_file_line = axis_atomic_ptr_load((void *)&posix_self->get_file_line);
