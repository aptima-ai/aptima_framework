 ...
 */
uint32_t elf_crc32_file(axis_backtrace_t *self, int descriptor,
  ...
  axis_mmap_t file_view;

  if (!axis_mmap_init(&file_view, descriptor, 0, st.st_size)) {
    return 0;
  }

  uint32_t ret =
      elf_crc32(0, (const unsigned char *)file_view.data, st.st_size);

  axis_mmap_deinit(&file_view);
