...
#include "include_internal/axis_utils/backtrace/platform/posix/config.h"
...
#include "include_internal/axis_utils/backtrace/platform/posix/linux/crc32.h"
#include "include_internal/axis_utils/backtrace/platform/posix/linux/debugfile.h"
#include "include_internal/axis_utils/backtrace/platform/posix/linux/uncompress.h"
#include "include_internal/axis_utils/backtrace/platform/posix/linux/view.h"
#include "include_internal/axis_utils/backtrace/platform/posix/linux/zlib.h"
...
static void elf_nosyms(axis_backtrace_t *self, uintptr_t addr,
                       axis_backtrace_dump_syminfo_func_t dump_file_line_cb,
 ...
 */
static int elf_nodebug(axis_backtrace_t *self_, uintptr_t pc,
                       axis_backtrace_dump_file_line_func_t callback,
                       axis_backtrace_error_func_t error_cb, void *data) {
  ...
  if (self->get_syminfo != NULL && self->get_syminfo != elf_nosyms) {
    ...
    self->get_syminfo((axis_backtrace_t *)self, pc,
...
static int elf_initialize_syminfo(axis_backtrace_t *self, uintptr_t base_address,
  ...
  elf_symbols =
      (struct elf_symbol *)axis_malloc_without_backtrace(elf_symbol_size);
  ...
  for (i = 0; i < sym_count; ++i, ++sym) {
    ...
    if (sym->st_name >= strtab_size) {
      ...
      axis_free_without_backtrace(elf_symbols);
      return 0;
    }
...
static void elf_add_syminfo_data(axis_backtrace_t *self_,
  ...
  while (1) {
    ...
    while (1) {
      struct elf_syminfo_data *p = axis_atomic_ptr_load((void *)pp);
      if (p == NULL) {
        break;
      }
...
static void elf_syminfo(axis_backtrace_t *self_, uintptr_t addr,
                        axis_backtrace_dump_syminfo_func_t callback,
                        axis_backtrace_error_func_t error_cb, void *data) {
  axis_backtrace_posix_t *self = (axis_backtrace_posix_t *)self_;
  assert(self && "Invalid argument.");
  ...
  struct elf_syminfo_data **pp =
      (struct elf_syminfo_data **)(void *)&self->get_syminfo_data;
  while (1) {
    struct elf_syminfo_data *edata = axis_atomic_ptr_load((void *)pp);
    if (edata == NULL) {
      break;
    }
...
static int elf_uncompress_zdebug(axis_backtrace_t *self,
  ...
  *uncompressed_size = 0;
  ...
  if (*uncompressed != NULL && *uncompressed_size >= sz)
    ...
    po = (unsigned char *)axis_malloc_without_backtrace(sz);
    if (po == NULL) {
      return 0;
    }
  }
...
static int elf_uncompress_chdr(axis_backtrace_t *self,
                               const unsigned char *compressed,
                               size_t compressed_size, uint16_t *zdebug_table,
                               axis_backtrace_error_func_t error_cb, void *data,
  ...
  *uncompressed_size = 0;
  ...
  if (*uncompressed != NULL && *uncompressed_size >= chdr->ch_size)
    ...
    alc = axis_malloc_without_backtrace(alc_len);
    if (alc == NULL) {
      return 0;
    }
...
skip:
  if (alc != NULL && alc_len > 0) axis_free_without_backtrace(alc);
  return 1;
}
...
static int elf_uncompress_lzma(axis_backtrace_t *self,
                               const unsigned char *compressed,
                               size_t compressed_size,
                               axis_backtrace_error_func_t error_cb, void *data,
                               unsigned char **uncompressed,
  ...
  mem = (unsigned char *)axis_malloc_without_backtrace(index_uncompressed_size);
  ...
  *uncompressed_size = index_uncompressed_size;
  ...
  probs = (uint16_t *)axis_malloc_without_backtrace(LZMA_PROB_TOTAL_COUNT *
                                                   sizeof(uint16_t));
  if (unlikely(probs == NULL)) {
    axis_free_without_backtrace(mem);
    return 0;
  }
 ...
 * determined.
 */
static int elf_add(axis_backtrace_t *self, const char *filename, int descriptor,
                   const unsigned char *memory, size_t memory_size,
                   uintptr_t base_address, axis_backtrace_error_func_t error_cb,
...
#define BACKTRACE_ELFCLASS ELFCLASS64
  ...
  if (symtab_shndx != 0 && !debuginfo) {
    ...
    sdata =
        (struct elf_syminfo_data *)axis_malloc_without_backtrace(sizeof *sdata);
    if (sdata == NULL) {
      goto fail;
    }
    ...
    if (!elf_initialize_syminfo(self, base_address, symtab_view.view.data,
                                ...
                                opd)) {
      axis_free_without_backtrace(sdata);
      goto fail;
    }
  ...
  for (i = 0; i < (int)DEBUG_MAX; ++i) {
    if (sections[i].size == 0 && zsections[i].size > 0) {
      ...
      if (zdebug_table == NULL) {
        zdebug_table =
            (uint16_t *)axis_malloc_without_backtrace(ZLIB_TABLE_SIZE);
        if (zdebug_table == NULL) {
          goto fail;
        }
      }
  ...
  if (zdebug_table != NULL) {
    axis_free_without_backtrace(zdebug_table);
    zdebug_table = NULL;
  }
  ...
  for (i = 0; i < (int)DEBUG_MAX; ++i) {
    ...
    if (zdebug_table == NULL) {
      zdebug_table =
          (uint16_t *)axis_malloc_without_backtrace(ZDEBUG_TABLE_SIZE);
      if (zdebug_table == NULL) {
        goto fail;
      }
    }
  ...
  if (zdebug_table != NULL) axis_free_without_backtrace(zdebug_table);
...
static int
  ...
  axis_backtrace_get_file_line_func_t elf_fileline_fn;
  int found_dwarf;
  ...
  if (info->dlpi_name == NULL || info->dlpi_name[0] == '\0') {
    ...
    if (pd->exe_descriptor != -1) {
      axis_file_close(pd->exe_descriptor);
      pd->exe_descriptor = -1;
    }

    filename = info->dlpi_name;
    descriptor = axis_file_open(info->dlpi_name, &does_not_exist);
    if (descriptor < 0) {
      return 0;
    }
  }
 ...
 */
int axis_backtrace_init_posix(axis_backtrace_t *self_, const char *filename,
                             int descriptor,
                             axis_backtrace_error_func_t error_cb, void *data,
                             axis_backtrace_get_file_line_func_t *fileline_fn) {
  axis_backtrace_posix_t *self = (axis_backtrace_posix_t *)self_;
  ...
  int found_sym;
  int found_dwarf;
  axis_backtrace_get_file_line_func_t elf_fileline_fn = elf_nodebug;
  struct phdr_data pd;
  ...
  if (found_sym) {
    axis_atomic_ptr_store((void *)&self->get_syminfo, elf_syminfo);
  } else {
    (void)__sync_bool_compare_and_swap(&self->get_syminfo, NULL, elf_nosyms);
  }

  *fileline_fn = (axis_backtrace_get_file_line_func_t)axis_atomic_ptr_load(
