...
typedef struct unit {
  ...
  /* PC to line number mapping.  This is NULL if the values have not
     ...
     reading the values.  */
  struct line *lines;
  axis_atomic_t lines_count;  // Number of entries in lines.

  // PC ranges to function.
  struct function_addrs *function_addrs;
  axis_atomic_t function_addrs_count;
} unit;
...
static void dwarf_buf_error(axis_backtrace_t *self, struct dwarf_buf *buf,
...
/* Require at least COUNT bytes in BUF.  Return 1 if all is well, 0 on
   error.  */
static int require(axis_backtrace_t *self, struct dwarf_buf *buf, size_t count) {
  if (buf->left >= count) {
    return 1;
  }
...
/* Advance COUNT bytes in BUF.  Return 1 if all is well, 0 on
   error.  */
static int advance(axis_backtrace_t *self, struct dwarf_buf *buf, size_t count) {
...
static const char *read_string(axis_backtrace_t *self, struct dwarf_buf *buf) {
...
static unsigned char read_byte(axis_backtrace_t *self, struct dwarf_buf *buf) {
...
static signed char read_sbyte(axis_backtrace_t *self, struct dwarf_buf *buf) {
  const unsigned char *p = buf->buf;
...
static uint16_t read_uint16(axis_backtrace_t *self, struct dwarf_buf *buf) {
  const unsigned char *p = buf->buf;
...
static uint32_t read_uint24(axis_backtrace_t *self, struct dwarf_buf *buf) {
  const unsigned char *p = buf->buf;
...
static uint32_t read_uint32(axis_backtrace_t *self, struct dwarf_buf *buf) {
  const unsigned char *p = buf->buf;
...
static uint64_t read_uint64(axis_backtrace_t *self, struct dwarf_buf *buf) {
  const unsigned char *p = buf->buf;
...
/* Read an offset from BUF and advance the appropriate number of
   bytes.  */
static uint64_t read_offset(axis_backtrace_t *self, struct dwarf_buf *buf,
                            int is_dwarf64) {
  if (is_dwarf64) {
...
/* Read an address from BUF and advance the appropriate number of
   bytes.  */
static uint64_t read_address(axis_backtrace_t *self, struct dwarf_buf *buf,
                             int addrsize) {
  switch (addrsize) {
    case 1:
...
static uint64_t read_uleb128(axis_backtrace_t *self, struct dwarf_buf *buf) {
  uint64_t ret;
  unsigned int shift;
  int overflow;
  unsigned char b;
...
static int64_t read_sleb128(axis_backtrace_t *self, struct dwarf_buf *buf) {
  uint64_t val;
  unsigned int shift;
  int overflow;
  unsigned char b;
...
static uint64_t read_initial_length(axis_backtrace_t *self,
...
static void free_abbrevs(axis_backtrace_t *self, struct abbrevs *abbrevs,
                         axis_backtrace_error_func_t error_cb, void *data) {
  size_t i;

  for (i = 0; i < abbrevs->num_abbrevs; ++i) {
    axis_free_without_backtrace(abbrevs->abbrevs[i].attrs);
  }
  axis_free_without_backtrace(abbrevs->abbrevs);
...
static int read_attribute(axis_backtrace_t *self, enum dwarf_form form,
...
static int resolve_string(axis_backtrace_t *self,
                          const struct dwarf_sections *dwarf_sections,
  ...
  switch (val->encoding) {
    ...
    case ATTR_VAL_STRING_INDEX: {
      ...
      if (offset >= dwarf_sections->size[DEBUG_STR]) {
        dwarf_buf_error(self, &offset_buf, "DW_FORM_strx offset out of range",
                        0);
        return 0;
      }
...
static int resolve_addr_index(axis_backtrace_t *self,
                              const struct dwarf_sections *dwarf_sections,
...
static int add_unit_addr(axis_backtrace_t *self, void *rdata, uintptr_t lowpc,
                         uintptr_t highpc, axis_backtrace_error_func_t error_cb,
                         void *data, void *pvec) {
  ...
  p = (unit_addrs *)axis_vector_grow(&vec->vec, sizeof(unit_addrs));
  if (p == NULL) {
    return 0;
  }
...
static int read_abbrevs(axis_backtrace_t *self, uint64_t abbrev_offset,
  ...
  abbrevs->abbrevs = (struct abbrev *)axis_malloc_without_backtrace(
  ...
  while (1) {
    ...
    if (num_attrs == 0) {
      ...
      attrs = (struct attr *)axis_malloc_without_backtrace(num_attrs *
                                                          sizeof *attrs);
      if (attrs == NULL) {
        goto fail;
      }
...
static const struct abbrev *lookup_abbrev(axis_backtrace_t *self,
                                          struct abbrevs *abbrevs,
                                          uint64_t code,
                                          axis_backtrace_error_func_t error_cb,
                                          void *data) {
...
static int add_low_high_range(
    axis_backtrace_t *self, const struct dwarf_sections *dwarf_sections,
    uintptr_t base_address, int is_bigendian, struct unit *u,
    const struct pcrange *pcrange,
    int (*add_range)(axis_backtrace_t *self, void *rdata, uintptr_t lowpc,
                     uintptr_t highpc, axis_backtrace_error_func_t error_cb,
                     void *data, void *vec),
    void *rdata, axis_backtrace_error_func_t error_cb, void *data, void *vec) {
  uintptr_t lowpc;
  uintptr_t highpc;
...
static int add_ranges(axis_backtrace_t *self,
                      const struct dwarf_sections *dwarf_sections,
                      uintptr_t base_address, int is_bigendian, struct unit *u,
                      uintptr_t base, const struct pcrange *pcrange,
                      int (*add_range)(axis_backtrace_t *self, void *rdata,
                                       uintptr_t lowpc, uintptr_t highpc,
                                       axis_backtrace_error_func_t error_cb,
                                       void *data, void *vec),
                      void *rdata, axis_backtrace_error_func_t error_cb,
                      void *data, void *vec) {
...
static int find_address_ranges(axis_backtrace_t *self, uintptr_t base_address,
...
static int build_address_map(axis_backtrace_t *self, uintptr_t base_address,
                             const struct dwarf_sections *dwarf_sections,
                             int is_bigendian, struct dwarf_data *altlink,
                             axis_backtrace_error_func_t error_cb, void *data,
  ...
  while (info.left > 0) {
    ...
    pu = (struct unit **)axis_vector_grow(&units, sizeof(struct unit *));
    if (pu == NULL) {
      goto fail;
    }

    u = (struct unit *)axis_malloc_without_backtrace(sizeof *u);
    if (u == NULL) {
      goto fail;
    }
  ...
  pa = (unit_addrs *)axis_vector_grow(&addrs->vec, sizeof(unit_addrs));
  if (pa == NULL) {
    goto fail;
  }
...
fail:
  if (units_count > 0) {
    ...
    for (i = 0; i < units_count; i++) {
      ...
      axis_free_without_backtrace(pu[i]);
    }
    axis_vector_deinit(&units);
  }
  if (addrs->count > 0) {
    axis_vector_deinit(&addrs->vec);
    addrs->count = 0;
  }
  return 0;
}
...
static int add_line(axis_backtrace_t *self, struct dwarf_data *ddata,
  ...
  ln = (line *)axis_vector_grow(&vec->vec, sizeof(line));
  if (ln == NULL) {
    return 0;
  }
...
static void free_line_header(axis_backtrace_t *self, struct line_header *hdr,
                             axis_backtrace_error_func_t error_cb, void *data) {
  if (hdr->dirs_count != 0) {
    axis_free_without_backtrace(hdr->dirs);
  }
  axis_free_without_backtrace(hdr->filenames);
}
...
static int read_v2_paths(axis_backtrace_t *self, struct unit *u,
  ...
  ++hdr->dirs_count;
  hdr->dirs = (const char **)axis_malloc_without_backtrace(hdr->dirs_count *
                                                          sizeof(const char *));
  if (hdr->dirs == NULL) {
    return 0;
  }
  ...
  if (!advance(self, hdr_buf, 1)) return 0;
  ...
  ++hdr->filenames_count;
  hdr->filenames = (const char **)axis_malloc_without_backtrace(
  ...
  while (*hdr_buf->buf != '\0') {
    ...
    if (IS_ABSOLUTE_PATH(filename) ||
      ...
      s = (char *)axis_malloc_without_backtrace(dir_len + filename_len + 2);
      if (s == NULL) {
        return 0;
      }
...
static int read_lnct(axis_backtrace_t *self, struct dwarf_data *ddata,
  ...
  if (dir == NULL)
    *string = path;
    ...
    s = (char *)axis_malloc_without_backtrace(dir_len + path_len + 2);
    if (s == NULL) {
      return 0;
    }
...
static int read_line_header_format_entries(
    axis_backtrace_t *self, struct dwarf_data *ddata, struct unit *u,
  ...
  if (formats_count == 0) {
    ...
    formats = (struct line_header_format *)axis_malloc_without_backtrace(
  ...
  paths = (const char **)axis_malloc_without_backtrace(paths_count *
                                                      sizeof(const char *));
  if (paths == NULL) {
    ret = 0;
    goto exit;
  ...
  for (i = 0; i < paths_count; i++) {
    if (!read_lnct(self, ddata, u, hdr_buf, hdr, formats_count, formats,
                   &paths[i])) {
      axis_free_without_backtrace(paths);
      ret = 0;
      goto exit;
    }
  }
...
exit:
  if (formats != NULL) {
    axis_free_without_backtrace(formats);
  }
...
static int read_line_header(axis_backtrace_t *self, struct dwarf_data *ddata,
...
static int read_line_program(axis_backtrace_t *self, struct dwarf_data *ddata,
      ...
      switch (op) {
        ...
        case DW_LNE_define_file: {
          ...
          if (IS_ABSOLUTE_PATH(f))
            ...
            dir_len = strlen(dir);
            f_len = strlen(f);
            p = (char *)axis_malloc_without_backtrace(dir_len + f_len + 2);
            if (p == NULL) {
              return 0;
            }
...
static int read_line_info(axis_backtrace_t *self, struct dwarf_data *ddata,
                          axis_backtrace_error_func_t error_cb, void *data,
  ...
  if (!axis_vector_release_remaining_space(&vec.vec)) {
    goto fail;
  }
...
static const char *read_referenced_name(axis_backtrace_t *self,
                                        struct dwarf_data *, struct unit *,
                                        uint64_t, axis_backtrace_error_func_t,
                                        void *);
...
static int add_function_range(axis_backtrace_t *self, void *rdata,
                              uintptr_t lowpc, uintptr_t highpc,
                              axis_backtrace_error_func_t error_cb, void *data,
                              void *pvec) {
  ...
  p = (function_addrs *)axis_vector_grow(&vec->vec, sizeof(function_addrs));
  if (p == NULL) {
    return 0;
  }
...
static int read_function_entry(axis_backtrace_t *self, struct dwarf_data *ddata,
  ...
  while (unit_buf->left > 0) {
    ...
    if (is_function) {
      function =
          (struct function *)axis_malloc_without_backtrace(sizeof *function);
      if (function == NULL) {
        return 0;
      }
    ...
    if (is_function && function->name == NULL) {
      axis_free_without_backtrace(function);
      is_function = 0;
    }
      ...
      if (!is_function) {
        ...
        if (fvec.count > 0) {
          ...
          /* Allocate a trailing entry, but don't include it
             in fvec.count.  */
          p = (function_addrs *)axis_vector_grow(&fvec.vec,
          ...
          if (!axis_vector_release_remaining_space(&fvec.vec)) {
            return 0;
          }
 ...
 */
static void read_function_info(axis_backtrace_t *self, struct dwarf_data *ddata,
                               const struct line_header *lhdr,
                               axis_backtrace_error_func_t error_cb, void *data,
  ...
  p = (function_addrs *)axis_vector_grow(&pfvec->vec, sizeof(function_addrs));
  if (p == NULL) {
    return;
  }
  ...
  if (fvec == NULL) {
    if (!axis_vector_release_remaining_space(&lvec.vec)) {
      return;
    }
    ...
    addrs = (function_addrs *)axis_vector_take_out(&fvec->vec);
    if (addrs == NULL) {
      return;
    }
...
static int report_inlined_functions(
    axis_backtrace_t *self, uintptr_t pc, struct function *function,
    axis_backtrace_dump_file_line_func_t dump_file_line_cb, void *data,
...
static int dwarf_lookup_pc(
    axis_backtrace_t *self, struct dwarf_data *ddata, uintptr_t pc,
    axis_backtrace_dump_file_line_func_t dump_file_line_cb,
    axis_backtrace_error_func_t error_cb, void *data, int *found) {
  ...
  *found = 1;
  ...
  while (entry > ddata->addrs && pc >= (entry - 1)->low &&
         pc < (entry - 1)->high) {
    lines = (struct line *)axis_atomic_ptr_load((void *)&u->lines);
  ...
  lines = axis_atomic_ptr_load((void *)&u->lines);
  ...
  if (lines == NULL) {
    ...
    axis_atomic_store(&u->lines_count, count);
    axis_atomic_store(&u->function_addrs_count, function_addrs_count);
    axis_atomic_ptr_store((void *)&u->function_addrs, function_addrs);
    axis_atomic_ptr_store((void *)&u->lines, lines);
  }
 ...
 * built earlier.
 */
static int dwarf_fileline(axis_backtrace_t *self_, uintptr_t pc,
                          axis_backtrace_dump_file_line_func_t dump_file_line_cb,
                          axis_backtrace_error_func_t error_cb, void *data) {
  axis_backtrace_posix_t *self = (axis_backtrace_posix_t *)self_;

  struct dwarf_data **pp = (struct dwarf_data **)&self->get_file_line_data;
  while (1) {
    struct dwarf_data *ddata = axis_atomic_ptr_load((void *)pp);
    if (ddata == NULL) {
      break;
    }
...
static struct dwarf_data *build_dwarf_data(
    ...
    struct dwarf_data *altlink, axis_backtrace_error_func_t error_cb,
    void *data) {
  ...
  if (!axis_vector_release_remaining_space(&addrs_vec.vec)) {
    return NULL;
  }

  if (!axis_vector_release_remaining_space(&units_vec.vec)) {
    return NULL;
  }
  ...
  fdata = (struct dwarf_data *)axis_malloc_without_backtrace(
 ...
 */
int backtrace_dwarf_add(axis_backtrace_t *self_, uintptr_t base_address,
                        const struct dwarf_sections *dwarf_sections,
                        int is_bigendian, struct dwarf_data *fileline_altlink,
                        axis_backtrace_error_func_t error_cb, void *data,
                        axis_backtrace_get_file_line_func_t *fileline_fn,
  ...
  while (1) {
    ...
    while (1) {
      struct dwarf_data *p = axis_atomic_ptr_load((void *)pp);
      if (p == NULL) {
        break;
      }
