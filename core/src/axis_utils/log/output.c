...
static void axis_log_output_set(axis_log_t *self,
                               const axis_log_output_func_t output_cb,
                               const axis_log_close_func_t close_cb,
                               void *user_data) {
...
static void axis_log_close_file_cb(void *user_data) {
  int *fd = user_data;
...
static int *get_log_fd(const char *log_path) {
  ...
  *fd_ptr = axis_file_get_fd(fp);
  return fd_ptr;
}

void axis_log_output_to_file_cb(axis_string_t *msg, void *user_data) {
  ...
  axis_string_append_formatted(msg, "%s", axis_LOG_EOL);
  ...
  DWORD written;
  WriteFile(handle, axis_string_get_raw_str(msg), (DWORD)axis_string_len(msg),
            &written, 0);
#else
  ...
  ssize_t bytes_written =
      write(fd, axis_string_get_raw_str(msg), axis_string_len(msg));
  if (bytes_written == -1) {
    perror("write failed");
  } else if ((size_t)bytes_written < axis_string_len(msg)) {
    (void)fprintf(stderr, "Partial write: only %zd of %zu bytes written\n",
                  bytes_written, axis_string_len(msg));
  }
#endif
}

void axis_log_set_output_to_file(axis_log_t *self, const char *log_path) {
  ...
  axis_log_output_set(self, axis_log_output_to_file_cb, axis_log_close_file_cb,
                     fd);

  axis_log_set_formatter(self, axis_log_default_formatter, NULL);
}

void axis_log_output_to_stderr_cb(axis_string_t *msg, void *user_data) {
  assert(msg && "Invalid argument.");
  ...
  DWORD written;
  WriteFile(GetStdHandle(STD_ERROR_HANDLE), axis_string_get_raw_str(msg),
            (DWORD)axis_string_len(msg), &written, 0);
#else
  // TODO(Wei): write() is atomic for buffers less than or equal to PIPE_BUF,
  // therefore we need to have some locking mechanism here to prevent log
  // interleaved.
  ssize_t bytes_written =
      write(STDERR_FILENO, axis_string_get_raw_str(msg), axis_string_len(msg));
  if (bytes_written == -1) {
    perror("write failed");
  } else if ((size_t)bytes_written < axis_string_len(msg)) {
    (void)fprintf(stderr, "Partial write: only %zd of %zu bytes written\n",
...
void axis_log_set_output_to_stderr(axis_log_t *self) {
  axis_log_output_set(self, axis_log_output_to_stderr_cb, NULL, NULL);

#if defined(OS_LINUX) || defined(OS_MACOS)
  axis_log_set_formatter(self, axis_log_colored_formatter, NULL);
  // axis_log_set_formatter(self, axis_log_default_formatter, NULL);
#else
  axis_log_set_formatter(self, axis_log_default_formatter, NULL);
#endif
...
void axis_log_output_to_file_deinit(axis_log_t *self) {
  assert(self && "Invalid argument.");
  assert(self->output.output_cb == axis_log_output_to_file_cb &&
         "Invalid argument.");

  free(self->output.user_data);
}

bool axis_log_is_output_to_file(axis_log_t *self) {
  assert(self && "Invalid argument.");
  return self->output.output_cb == axis_log_output_to_file_cb;
}
