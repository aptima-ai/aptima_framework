...
void axis_log_get_time(struct tm *time_info, size_t *msec) {
  ...
  *msec = st.wMilliseconds;
  ...
  if (!axis_log_time_cache_get(&tv, time_info)) {
    (void)localtime_r(&tv.tv_sec, time_info);
    axis_log_time_cache_set(&tv, time_info);
  }
...
void axis_log_add_time_string(axis_string_t *buf, struct tm *time_info,
                             size_t msec) {
  assert(buf && "Invalid argument.");

  axis_string_reserve(buf, TIME_LOG_SIZE);
  ...
  axis_string_append_formatted(buf, ".%03zu", msec);
}
