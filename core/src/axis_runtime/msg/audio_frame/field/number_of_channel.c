...
void axis_audio_frame_copy_number_of_channel(
  ...
  axis_raw_audio_frame_set_number_of_channel(
      (axis_audio_frame_t *)self,
      axis_raw_audio_frame_get_number_of_channel((axis_audio_frame_t *)src));
}

bool axis_audio_frame_process_number_of_channel(
    axis_msg_t *self, axis_raw_msg_process_one_field_func_t cb, void *user_data,
    axis_error_t *err) {
  axis_ASSERT(self && axis_raw_msg_check_integrity(self), "Should not happen.");

  axis_msg_field_process_data_t number_of_channel_field;
  axis_msg_field_process_data_init(
      &number_of_channel_field, axis_STR_NUMBER_OF_CHANNEL,
      &((axis_audio_frame_t *)self)->number_of_channel, false);
