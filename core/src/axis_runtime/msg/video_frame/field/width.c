...
void axis_video_frame_copy_width(axis_msg_t *self, axis_msg_t *src,
  ...
  axis_raw_video_frame_set_width(
      (axis_video_frame_t *)self,
      axis_raw_video_frame_get_width((axis_video_frame_t *)src));
}

bool axis_video_frame_process_width(axis_msg_t *self,
                                   axis_raw_msg_process_one_field_func_t cb,
                                   void *user_data, axis_error_t *err) {
  axis_ASSERT(self && axis_raw_msg_check_integrity(self), "Should not happen.");

  axis_msg_field_process_data_t width_field;
  axis_msg_field_process_data_init(&width_field, axis_STR_WIDTH,
                                  &((axis_video_frame_t *)self)->width, false);
