...
void axis_cmd_result_copy_is_final(axis_msg_t *self, axis_msg_t *src,
  ...
  axis_raw_cmd_result_set_final(
      (axis_cmd_result_t *)self,
      axis_raw_cmd_result_is_final((axis_cmd_result_t *)src, NULL), NULL);
}

bool axis_cmd_result_process_is_final(axis_msg_t *self,
                                     axis_raw_msg_process_one_field_func_t cb,
                                     void *user_data, axis_error_t *err) {
  axis_ASSERT(self && axis_raw_msg_check_integrity(self), "Should not happen.");

  axis_msg_field_process_data_t is_final_field;
  axis_msg_field_process_data_init(&is_final_field, axis_STR_IS_FINAL,
                                  &((axis_cmd_result_t *)self)->is_final, false);
