...
void axis_cmd_base_copy_seq_id(axis_msg_t *self, axis_msg_t *src,
  ...
  axis_string_set_formatted(
      axis_value_peek_string(&((axis_cmd_base_t *)self)->seq_id), "%s",
      axis_value_peek_raw_str(&((axis_cmd_base_t *)src)->seq_id, NULL));
}

bool axis_cmd_base_process_seq_id(axis_msg_t *self,
                                 axis_raw_msg_process_one_field_func_t cb,
                                 void *user_data, axis_error_t *err) {
  axis_ASSERT(self && axis_raw_msg_check_integrity(self), "Should not happen.");

  axis_msg_field_process_data_t seq_id_field;
  axis_msg_field_process_data_init(&seq_id_field, axis_STR_SEQ_ID,
                                  &((axis_cmd_base_t *)self)->seq_id, false);
