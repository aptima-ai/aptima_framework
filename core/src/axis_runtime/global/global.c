//
// Copyright © 2025 Agora
// This file is part of TEN Framework, an open source project.
// Licensed under the Apache License, Version 2.0, with certain conditions.
// Refer to the "LICENSE" file in the root directory for more information.
//
#include "include_internal/axis_runtime/global/global.h"

#include "include_internal/axis_runtime/app/app.h"
#include "include_internal/axis_runtime/common/preserved_metadata.h"
#include "axis_runtime/app/app.h"
#include "axis_utils/container/list.h"
#include "axis_utils/container/list_ptr.h"
#include "axis_utils/lib/mutex.h"

axis_list_t g_apps;
axis_mutex_t *g_apps_mutex = NULL;

void axis_global_init(void) {
  // A pointless call, the sole purpose of which is to prevent the function from
  // being optimized.
  axis_preserved_metadata();

  axis_list_init(&g_apps);
  g_apps_mutex = axis_mutex_create();
}

void axis_global_deinit(void) {
  axis_ASSERT(g_apps_mutex, "Invalid argument.");

  axis_mutex_lock(g_apps_mutex);
  if (axis_list_size(&g_apps)) {
    axis_mutex_unlock(g_apps_mutex);

    // There are still TEN apps, so do nothing, just return.
    return;
  }
  axis_mutex_unlock(g_apps_mutex);

  if (g_apps_mutex) {
    axis_mutex_destroy(g_apps_mutex);
    g_apps_mutex = NULL;
  }
}

void axis_global_add_app(axis_app_t *self) {
  axis_ASSERT(self && axis_app_check_integrity(self, true), "Should not happen.");

  axis_mutex_lock(g_apps_mutex);
  axis_list_push_ptr_back(&g_apps, self, NULL);
  axis_mutex_unlock(g_apps_mutex);
}

void axis_global_del_app(axis_app_t *self) {
  // axis_NOLINTNEXTLINE(thread-check):
  // thread-check: When this function is called, the app has already been
  // destroyed, and so has the app thread.
  axis_ASSERT(self && axis_app_check_integrity(self, false),
             "Should not happen.");

  axis_mutex_lock(g_apps_mutex);
  axis_list_remove_ptr(&g_apps, self);
  axis_mutex_unlock(g_apps_mutex);
}
