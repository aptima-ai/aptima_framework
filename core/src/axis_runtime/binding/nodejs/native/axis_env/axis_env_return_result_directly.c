...
typedef struct axis_env_notify_return_result_directly_ctx_t {
  axis_shared_ptr_t *c_cmd_result;
  axis_nodejs_tsfn_t *js_cb;
} axis_env_notify_return_result_directly_ctx_t;

typedef struct axis_nodejs_return_result_directly_callback_call_ctx_t {
  axis_nodejs_tsfn_t *js_cb;
  axis_error_t *error;
} axis_nodejs_return_result_directly_callback_call_ctx_t;

static axis_env_notify_return_result_directly_ctx_t *
axis_env_notify_return_result_directly_ctx_create(axis_shared_ptr_t *c_cmd_result,
                                                 axis_nodejs_tsfn_t *js_cb) {
  axis_env_notify_return_result_directly_ctx_t *ctx =
      axis_MALLOC(sizeof(axis_env_notify_return_result_directly_ctx_t));
...
static void axis_env_notify_return_result_directly_ctx_destroy(
    axis_env_notify_return_result_directly_ctx_t *ctx) {
...
static axis_nodejs_return_result_directly_callback_call_ctx_t *
axis_nodejs_return_result_directly_callback_call_ctx_create(
    axis_nodejs_tsfn_t *js_cb, axis_error_t *error) {
  axis_nodejs_return_result_directly_callback_call_ctx_t *ctx =
      axis_MALLOC(sizeof(axis_nodejs_return_result_directly_callback_call_ctx_t));
  axis_ASSERT(ctx, "Failed to allocate memory.");
...
static void axis_nodejs_return_result_directly_callback_call_ctx_destroy(
    axis_nodejs_return_result_directly_callback_call_ctx_t *ctx) {
  axis_ASSERT(ctx, "Invalid argument.");
...
static void tsfn_proxy_return_result_directly_callback(napi_env env,
                                                       ...
                                                       void *context,
                                                       void *data) {
  axis_nodejs_return_result_directly_callback_call_ctx_t *ctx = data;
  axis_ASSERT(ctx, "Invalid argument.");

  axis_nodejs_tsfn_t *js_cb_tsfn = ctx->js_cb;
  axis_ASSERT(js_cb_tsfn && axis_nodejs_tsfn_check_integrity(js_cb_tsfn, false),
             "Should not happen.");
  ...
  axis_nodejs_return_result_directly_callback_call_ctx_destroy(ctx);
}

static void proxy_return_result_directly_error_callback(axis_env_t *self,
                                                        void *user_data,
                                                        axis_error_t *error) {
  axis_ASSERT(self && axis_env_check_integrity(self, true), "Should not happen.");

  axis_env_notify_return_result_directly_ctx_t *ctx = user_data;
  ...
  if (error) {
    ...
    axis_error_copy(cloned_error, error);
  }

  axis_nodejs_return_result_directly_callback_call_ctx_t *call_info =
      axis_nodejs_return_result_directly_callback_call_ctx_create(js_cb,
                                                                 cloned_error);
  ...
  axis_env_notify_return_result_directly_ctx_destroy(ctx);
}

static void axis_env_proxy_notify_return_result_directly(axis_env_t *axis_env,
                                                        void *user_data) {
  ...
  bool rc = axis_env_return_result_directly(
      axis_env, ctx->c_cmd_result, proxy_return_result_directly_error_callback,
      ctx, &err);
  if (!rc) {
    proxy_return_result_directly_error_callback(axis_env, ctx, &err);
  }

  axis_error_deinit(&err);
}

napi_value axis_nodejs_axis_env_return_result_directly(napi_env env,
  ...
  axis_env_notify_return_result_directly_ctx_t *notify_info =
      axis_env_notify_return_result_directly_ctx_create(
          axis_shared_ptr_clone(cmd_result_bridge->msg.msg), cb_tsfn);
  bool rc = axis_env_proxy_notify(axis_env_bridge->c_axis_env_proxy,
                                 axis_env_proxy_notify_return_result_directly,
  ...
  if (!rc) {
    ...
    axis_env_notify_return_result_directly_ctx_destroy(notify_info);
  }
