...
static void axis_env_proxy_notify_on_deinit_done(axis_env_t *axis_env,
  ...
  axis_env_proxy_t *axis_env_proxy = user_data;

  if (axis_env_proxy) {
    axis_ASSERT(axis_env_proxy_get_thread_cnt(axis_env_proxy, NULL) == 1,
               "Should not happen.");
...
napi_value axis_nodejs_axis_env_on_deinit_done(napi_env env,
  ...
  if (axis_env_bridge->c_axis_env->attach_to == axis_ENV_ATTACH_TO_ADDON) {
    rc = axis_env_on_deinit_done(axis_env_bridge->c_axis_env, &err);
  } else {
    rc = axis_env_proxy_notify_async(axis_env_bridge->c_axis_env_proxy,
                                    axis_env_proxy_notify_on_deinit_done,
                                    axis_env_bridge->c_axis_env_proxy, &err);
  }

  if (!rc) {
    axis_LOGD("APTIMA/JS failed to on_deinit_done.");
  ...
  // Release the reference to the JS axis_env object.
  uint32_t js_axis_env_ref_count = 0;
  status = napi_reference_unref(env, axis_env_bridge->bridge.js_instance_ref,
