...
napi_value axis_nodejs_axis_env_on_init_done(napi_env env,
  ...
  if (axis_env_bridge->c_axis_env->attach_to == axis_ENV_ATTACH_TO_ADDON) {
    rc = axis_env_on_init_done(axis_env_bridge->c_axis_env, &err);
  } else {
  ...
  if (!rc) {
    axis_LOGD("APTIMA/JS failed to on_init_done.");
