...
napi_value axis_nodejs_axis_env_on_start_done(napi_env env,
  ...
  if (axis_env_bridge->c_axis_env->attach_to == axis_ENV_ATTACH_TO_ADDON) {
    rc = axis_env_on_start_done(axis_env_bridge->c_axis_env, &err);
  } else {
    rc = axis_env_proxy_notify_async(axis_env_bridge->c_axis_env_proxy,
                                    axis_env_proxy_notify_on_start_done, NULL,
                                    &err);
  }
