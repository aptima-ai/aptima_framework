...
typedef struct axis_nodejs_send_audio_frame_callback_call_ctx_t {
  axis_nodejs_tsfn_t *js_cb;
  axis_error_t *error;
} axis_nodejs_send_audio_frame_callback_call_ctx_t;
...
static axis_nodejs_send_audio_frame_callback_call_ctx_t *
axis_nodejs_send_audio_frame_callback_call_ctx_create(axis_nodejs_tsfn_t *js_cb,
                                                     axis_error_t *error) {
  axis_nodejs_send_audio_frame_callback_call_ctx_t *ctx =
      axis_MALLOC(sizeof(axis_nodejs_send_audio_frame_callback_call_ctx_t));
  axis_ASSERT(ctx, "Failed to allocate memory.");
...
static void axis_nodejs_send_audio_frame_callback_call_ctx_destroy(
    axis_nodejs_send_audio_frame_callback_call_ctx_t *ctx) {
  axis_ASSERT(ctx, "Invalid argument.");
...
static void tsfn_proxy_send_audio_frame_callback(napi_env env, napi_value js_cb,
                                                 void *context,
                                                 void *audio_frame) {
  axis_nodejs_send_audio_frame_callback_call_ctx_t *ctx = audio_frame;
  ...
  axis_nodejs_send_audio_frame_callback_call_ctx_destroy(ctx);
}

static void proxy_send_audio_frame_callback(axis_env_t *axis_env,
  ...
  axis_env_notify_send_audio_frame_ctx_t *ctx = user_audio_frame;
  axis_ASSERT(ctx, "Should not happen.");
  ...
  axis_nodejs_send_audio_frame_callback_call_ctx_t *call_info =
      axis_nodejs_send_audio_frame_callback_call_ctx_create(js_cb, cloned_error);
  axis_ASSERT(call_info, "Failed to create callback call info.");

  bool rc = axis_nodejs_tsfn_invoke(ctx->js_cb, call_info);
  axis_ASSERT(rc, "Should not happen.");

  axis_env_notify_send_audio_frame_ctx_destroy(ctx);
}

static void axis_env_proxy_notify_send_audio_frame(axis_env_t *axis_env,
                                                  void *user_audio_frame) {
  axis_ASSERT(user_audio_frame, "Invalid argument.");
  axis_ASSERT(axis_env && axis_env_check_integrity(axis_env, true),
             "Should not happen.");
  ...
  bool rc = axis_env_send_audio_frame(
      axis_env, ctx->c_audio_frame, proxy_send_audio_frame_callback, ctx, &err);
  if (!rc) {
    proxy_send_audio_frame_callback(axis_env, NULL, ctx, &err);
  }

  axis_error_deinit(&err);
}

napi_value axis_nodejs_axis_env_send_audio_frame(napi_env env,
  ...
  axis_nodejs_tsfn_t *cb_tsfn =
      axis_nodejs_tsfn_create(env, "[TSFN] TenEnv::send_audio_frame callback",
