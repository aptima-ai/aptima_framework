...
napi_value axis_nodejs_axis_env_set_property_from_json(napi_env env,
                                                     napi_callback_info info) {
  const size_t argc = 4;
  napi_value args[argc];  // axis_env, path, json_str, callback
  ...
  axis_string_t property_value_json_str;
  axis_string_init(&property_value_json_str);

  rc = axis_nodejs_get_str_from_js(env, args[2], &property_value_json_str);
  RETURN_UNDEFINED_IF_NAPI_FAIL(rc, "Failed to get property value", NULL);

  axis_json_t *json = axis_json_from_string(
      axis_string_get_raw_str(&property_value_json_str), &err);
  if (!json) {
    ...
    axis_string_deinit(&property_value_json_str);
    axis_error_deinit(&err);
  }

  axis_value_t *value = axis_value_from_json(json);
  axis_ASSERT(value, "Failed to create value from JSON.");
