...
static void tsfn_proxy_get_property_to_json_callback(napi_env env,
  ...
  if (!value) {
    ...
    axis_json_t *value_json = axis_value_to_json(value);
    if (!value_json) {
      ...
      const char *json_str = axis_json_to_string(value_json, NULL, &must_free);
...
napi_value axis_nodejs_axis_env_get_property_to_json(napi_env env,
  ...
  axis_nodejs_tsfn_t *cb_tsfn =
      axis_nodejs_tsfn_create(env, "[TSFN] TenEnv::getPropertyToJson callback",
