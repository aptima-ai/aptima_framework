...
static void axis_env_proxy_notify_on_configure_done(axis_env_t *axis_env,
  ...
  bool rc = axis_env_on_configure_done(axis_env, &err);
  axis_ASSERT(rc, "Should not happen.");

  axis_error_deinit(&err);
}

napi_value axis_nodejs_axis_env_on_configure_done(napi_env env,
  ...
  if (axis_env_bridge->c_axis_env->attach_to == axis_ENV_ATTACH_TO_ADDON) {
    rc = axis_env_on_configure_done(axis_env_bridge->c_axis_env, &err);
  } else {
    rc = axis_env_proxy_notify_async(axis_env_bridge->c_axis_env_proxy,
                                    axis_env_proxy_notify_on_configure_done,
                                    NULL, &err);
  }

  if (!rc) {
    axis_LOGD("APTIMA/JS failed to on_configure_done.");
