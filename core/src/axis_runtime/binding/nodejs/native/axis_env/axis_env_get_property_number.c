...
static void tsfn_proxy_get_property_number_callback(napi_env env,
  ...
  if (!value) {
    ...
    js_res = axis_nodejs_create_value_number(env, value, &err);
    if (!js_res) {
      js_error = axis_nodejs_create_error(env, &err);
      ASSERT_IF_NAPI_FAIL(js_error, "Failed to create JS error", NULL);
    }
...
napi_value axis_nodejs_axis_env_get_property_number(napi_env env,
  ...
  axis_nodejs_tsfn_t *cb_tsfn =
      axis_nodejs_tsfn_create(env, "[TSFN] TenEnv::getPropertyNumber callback",
