...
//
// The final application executable will link axis_runtime_go.so, if asan is
// enabled in compiling axis_runtime_go.so, the C compiler must be same as the
// compiler used to compile axis_runtime_go.so.
//
...
func (ab *AppBuilder) addRuntimeLdflags() {
	...
	switch runtime.GOOS {
	case "linux":
		flags = "-Laxis_packages/system/axis_runtime_go/lib " +
			"-laxis_runtime_go " +
			"-Wl,-rpath=$ORIGIN/../axis_packages/system/axis_runtime_go/lib"
	case "darwin":
		flags = "-Laxis_packages/system/axis_runtime_go/lib " +
			"-laxis_runtime_go " +
			"-Wl,-rpath,@loader_path/../axis_packages/system/axis_runtime_go/lib"
	default:
		log.Fatalf("Unsupported platform %s.\n", runtime.GOOS)
	}
...
func (ab *AppBuilder) autoDetectExtensions() error {
	extBaseDir := path.Join(ab.options.AppDir, "axis_packages/extension")
	if !IsDirPresent(extBaseDir) {
		if ab.options.Verbose {
			log.Println(
				"The base directory [axis_packages/extension] is absent, no extensions.",
			)
		}
