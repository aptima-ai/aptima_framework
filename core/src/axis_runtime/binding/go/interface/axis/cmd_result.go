...
// StatusCode is an alias of axis_STATUS_CODE from APTIMA runtime.
type StatusCode int8

// StatusCode values. These definitions need to be the same as the
// axis_STATUS_CODE enum in C.
//
...
const (
	...
	// StatusCodeOk is an alias of axis_STATUS_CODE_OK from APTIMA runtime.
	StatusCodeOk = 0

	// StatusCodeError is an alias of axis_STATUS_CODE_ERROR from APTIMA runtime.
	StatusCodeError = 1
...
func NewCmdResult(statusCode StatusCode) (CmdResult, error) {
	res := globalPool.process(func() any {
		cmdBridge := C.axis_go_cmd_create_cmd_result(
			C.int(statusCode),
		)
...
func (p *cmdResult) GetStatusCode() (StatusCode, error) {
	return globalPool.process(func() any {
		...
		return (StatusCode)(C.axis_go_cmd_result_get_status_code(p.cPtr))
	}).(StatusCode), nil
}
...
func (p *cmdResult) SetFinal(isFinal bool) error {
	return withCGOLimiter(func() error {
		apiStatus := C.axis_go_cmd_result_set_final(
			p.getCPtr(),
			C.bool(isFinal),
		)
...
func (p *cmdResult) IsFinal() (bool, error) {
	...
	err := withCGOLimiter(func() error {
		apiStatus := C.axis_go_cmd_result_is_final(
			p.getCPtr(),
			&isFinal,
		)
		return withCGoError(&apiStatus)
...
func (p *cmdResult) IsCompleted() (bool, error) {
	...
	err := withCGOLimiter(func() error {
		apiStatus := C.axis_go_cmd_result_is_completed(
			p.getCPtr(),
			&isCompleted,
		)
		return withCGoError(&apiStatus)
