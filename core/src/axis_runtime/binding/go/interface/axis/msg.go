...
// MsgType is an alias of `axis_MSG_TYPE` from APTIMA runtime.
type MsgType uint8
...
type msg struct {
	// The cPtr in msg is actually a pointer to the C struct axis_go_msg_t.
	//
	// We prefer to keep the cPtr as C.uintptr_t which is an integer holds the
	// bit pattern of the pointer to the C struct axis_go_msg_t. The reason is as
	// follows.
	//
	// - The C.uintptr_t is big enough to hold the bit pattern of a pointer. So
	//   it's safe to convert between an uintptr_t and *axis_go_msg_t in the C
	//   world.
	//
	...
	//
	// 	 - It's safe because the axis_go_msg_t pointer is allocated in the C
	...
	//     *C.axis_go_msg_t between GO and C. Refer to the chapter
	//     "Incomplete type" in README.md. There is no memory allocation if
	//     passing a C.uintptr_t between GO and C.
	//
	// - All accesses to the axis_go_msg_t pointer are always happened in the C
...
func (p *msg) GetName() (string, error) {
	...
	err := withCGOLimiter(func() error {
		apiStatus := C.axis_go_msg_get_name(p.cPtr, &msgName)
