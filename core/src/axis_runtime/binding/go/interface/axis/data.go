...
// Data is the interface for the APTIMA data message.
type Data interface {
	Msg
...
func (p *data) AllocBuf(size int) error {
	...
	err := withCGOLimiter(func() error {
		apiStatus := C.axis_go_data_alloc_buf(p.getCPtr(), C.int(size))
...
func (p *data) LockBuf() ([]byte, error) {
	...
	err := withCGOLimiter(func() error {
		apiStatus := C.axis_go_data_lock_buf(
			p.getCPtr(),
			&bufAddr,
			&bufSize,
		)
...
func (p *data) UnlockBuf(buf *[]byte) error {
	...
	err := withCGOLimiter(func() error {
		apiStatus := C.axis_go_data_unlock_buf(
			p.getCPtr(),
...
func (p *data) GetBuf() ([]byte, error) {
	...
	err := withCGOLimiter(func() error {
		apiStatus := C.axis_go_data_get_buf_size(p.getCPtr(), &bufSize)
		return withCGoError(&apiStatus)
	})
	...
	err = withCGOLimiter(func() error {
		apiStatus := C.axis_go_data_get_buf(
			p.getCPtr(),
