 ...
 * returned by axis_go_msg_property_get_type_and_size. The string value will be
 ...
 * returned by axis_go_msg_property_get_type_and_size. The buffer value will be
 * copied to @a value.
 */
 ...
 *
 * @param bridge_addr The bit pattern of the pointer to the struct axis_go_msg_t.
 *
 * @param path The underlying buffer of the GO string, which is passed with
 * unsafe.Pointer in GO world, so the type of @a path is void*, not char*. Only
 * the read operation is permitted. And the buffer is managed by GO, do not read
 ...
 *
 * @param bridge_addr The bit pattern of the pointer to the axis_go_msg_t.
 * Reinterpret it as a pointer to the axis_go_msg_t in the C world.
 */
