...
func (p *app) Run(runInBackground bool) {
	C.axis_go_app_run(p.cPtr, C.bool(runInBackground))
}

func (p *app) Close() {
	C.axis_go_app_close(p.cPtr)
}

func (p *app) Wait() {
	C.axis_go_app_wait(p.cPtr)
}
...
func NewApp(
	...
	pApp.cPtr = (*C.axis_go_app_t)(
		unsafe.Pointer(C.axis_go_app_create(C.uintptr_t(appObjID))),
	)
