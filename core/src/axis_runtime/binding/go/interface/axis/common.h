...
// The axis_go_handle_t is the C type corresponding to the `goHandle` in the GO
...
typedef uintptr_t axis_go_handle_t;
typedef struct axis_error_t axis_error_t;

typedef struct axis_go_handle_array_t {
  axis_go_handle_t *array;
  size_t size;
} axis_go_handle_array_t;

typedef struct axis_go_bridge_t {
  ...
  axis_shared_ptr_t *sp_ref_by_go;

  // This is a handle to the Go instance.
  axis_go_handle_t go_instance;
} axis_go_bridge_t;

// Return type for functions invoked from GO. The `axis_go_error_t` should
...
typedef struct axis_go_error_t {
  // The errno is always 0 if no error.
  // The type of this field must equal to the errno of axis_error_t.
  int64_t err_no;
  ...
  //
  // Note that the max size of err_msg is axis_GO_STATUS_ERR_MSG_BUF_SIZE - 1, so
  // the type is uint8_t here.
  uint8_t err_msg_size;

  // The err_msg is always NULL if no error.
  //
  // All functions invoked from GO return a `axis_go_error_t` instance, not a
  ...
  //
  //   Disadvantages: The size of `axis_go_error_t` is 264, which is too large.
  //   And as the `err_msg` is always allocated on the stack and the
  //   `axis_go_error_t` is returned by value not reference, CGO will allocate
  //   a piece of memory with the same size of `axis_go_error_t`, no matter the
  //   `err_msg` is empty or not. In other words, 264 bytes will be allocated in
  //   each cgo call.
  //
  //   Note that we can retrieve the size of `axis_go_error_t` using
  //   `unsafe.Sizeof(C.axis_go_error_t{})` in GO.
  //
  // - If the declaration is `char* err_msg`.
  //
  //   Advantages: The size of `axis_go_error_t` is 16. And as the `err_msg` is
  //   always allocated on the heap, no memory is allocated in returning
  //   `axis_go_error_t` from C to GO through CGO (after compilation
  //   optimization).
  //
  ...
  // `axis_go_error_t` in cgo functions is as follows.
  //
  ...
  //   MOVQ 0(BP), BP
  //
  // The first line loads the type of `axis_go_error_t` into AX, and the second
  ...
  //  }
  //
  // The first parameter `typ.size` is the size of `axis_go_error_t`.
  //
  // Why is there memory allocation (i.e., CALL runtime.newobject) during cgo
  // call if the declaration is `char err_msg[256]`?
  //
  // * Cgo will generate a GO type corresponding to `axis_go_error_t` in C, ex:
  //
  //   type _Ctype_struct_axis_go_status_t struct {
  //     err_no    _Ctype_int
  ...
  //   }
  //
  //   From the GO side, any C function returns `axis_go_error_t`, there will be
  //   a corresponding GO function returns `_Ctype_struct_axis_go_status_t`.
  //   Refer to chapter `cgo` in README.md for more details.
  //
  // * The C functions return `axis_go_error_t` by value, not reference. So the
  //   GO stack frame shall be prepared before calling C functions, and the
  //   memory for returned value will be allocated, i.e.,
  //   sizeof(_Ctype_struct_axis_go_status_t).
  char *err_msg;
} axis_go_error_t;
 ...
 */
axis_go_error_t axis_go_copy_c_str_to_slice_and_free(const char *src, void *dest);
