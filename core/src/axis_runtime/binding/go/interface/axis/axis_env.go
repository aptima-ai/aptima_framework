...
func (p *tenEnv) sendCmd(cmd Cmd, handler ResultHandler) error {
	...
	cStatus := C.axis_go_axis_env_send_cmd(
		p.cPtr,
		cmd.getCPtr(),
		cHandle(cb),
		C.bool(false),
...
func (p *tenEnv) SendData(data Data, handler ErrorHandler) error {
	...
	err := withCGO(func() error {
		apiStatus := C.axis_go_axis_env_send_data(
			p.cPtr,
			data.getCPtr(),
			cHandle(cb),
		)
...
func (p *tenEnv) SendVideoFrame(
	...
	err := withCGO(func() error {
		apiStatus := C.axis_go_axis_env_send_video_frame(
			p.cPtr,
			videoFrame.getCPtr(),
			cHandle(cb),
		)
...
func (p *tenEnv) SendAudioFrame(
	...
	err := withCGO(func() error {
		apiStatus := C.axis_go_axis_env_send_audio_frame(
			p.cPtr,
			audioFrame.getCPtr(),
			cHandle(cb),
		)
...
func (p *tenEnv) OnConfigureDone() error {
	...
	C.axis_go_axis_env_on_configure_done(p.cPtr)
...
func (p *tenEnv) OnInitDone() error {
	...
	C.axis_go_axis_env_on_init_done(p.cPtr)

	return nil
}

func (p *tenEnv) OnStartDone() error {
	C.axis_go_axis_env_on_start_done(p.cPtr)

	return nil
}

func (p *tenEnv) OnStopDone() error {
	C.axis_go_axis_env_on_stop_done(p.cPtr)

	return nil
}

func (p *tenEnv) OnDeinitDone() error {
	C.axis_go_axis_env_on_deinit_done(p.cPtr)
...
func (p *tenEnv) OnCreateInstanceDone(instance any, context uintptr) error {
	...
	case *extension:
		C.axis_go_axis_env_on_create_instance_done(p.cPtr, instance.cPtr, C.uintptr_t(context))
	default:
...
func (p *tenEnv) String() string {
	cString := C.axis_go_axis_env_debug_info(p.cPtr)
