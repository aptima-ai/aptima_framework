...
static void proxy_send_xxx_callback(axis_env_tester_t *self,
  ...
  axis_ASSERT(self && axis_env_tester_check_integrity(self),
             "Should not happen.");
  axis_ASSERT(user_video_frame, "Should not happen.");
...
PyObject *axis_py_axis_env_tester_send_video_frame(PyObject *self,
                                                 PyObject *args) {
  ...
  if (PyTuple_GET_SIZE(args) != 2) {
    return axis_py_raise_py_value_error_exception(
        "Invalid argument count when axis_env_tester.send_video_frame.");
  }
  ...
  axis_py_video_frame_t *py_video_frame = NULL;
  PyObject *cb_func = NULL;

  if (!PyArg_ParseTuple(args, "O!O", axis_py_video_frame_py_type(),
  ...
  if (cb_func) {
    ...
    success = axis_env_tester_send_video_frame(
        py_axis_env_tester->c_axis_env_tester, py_video_frame->msg.c_msg,
        proxy_send_xxx_callback, cb_func, &err);
  } else {
    success = axis_env_tester_send_video_frame(
        py_axis_env_tester->c_axis_env_tester, py_video_frame->msg.c_msg, NULL,
        NULL, &err);
  }
  ...
  if (!success) {
    ...
    axis_py_raise_py_runtime_error_exception("Failed to send video_frame.");
    goto done;
  } else {
