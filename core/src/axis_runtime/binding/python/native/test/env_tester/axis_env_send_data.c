...
static void proxy_send_xxx_callback(axis_env_tester_t *self, void *user_data,
                                    axis_error_t *error) {
...
PyObject *axis_py_axis_env_tester_send_data(PyObject *self, PyObject *args) {
  ...
  if (PyTuple_GET_SIZE(args) != 2) {
    return axis_py_raise_py_value_error_exception(
        "Invalid argument count when axis_env_tester.send_data.");
  }
  ...
  if (cb_func) {
    ...
    success = axis_env_tester_send_data(py_axis_env_tester->c_axis_env_tester,
                                       py_data->msg.c_msg,
                                       proxy_send_xxx_callback, cb_func, &err);
  } else {
    success = axis_env_tester_send_data(py_axis_env_tester->c_axis_env_tester,
                                       py_data->msg.c_msg, NULL, NULL, &err);
  }
