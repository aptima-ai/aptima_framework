...
static void axis_env_proxy_notify_send_video_frame(axis_env_t *axis_env,
                                                  void *notify_info_) {
  axis_ASSERT(notify_info_, "Invalid argument.");
  axis_ASSERT(axis_env && axis_env_check_integrity(axis_env, true),
             "Should not happen.");

  axis_env_notify_send_video_frame_ctx_t *notify_info = notify_info_;
...
PyObject *axis_py_axis_env_send_video_frame(PyObject *self, PyObject *args) {
  ...
  axis_shared_ptr_t *cloned_video_frame =
      axis_shared_ptr_clone(py_video_frame->msg.c_msg);
  axis_env_notify_send_video_frame_ctx_t *notify_info =
      axis_env_notify_send_video_frame_ctx_create(cloned_video_frame, cb_func);
