...
const TerminalPopup: React.FC<TerminalPopupProps> = ({ data, onClose }) => {
    ...
    ws.current.onmessage = async (event) => {
      ...
      if (event.data instanceof Blob) {
        try {
          ...
          console.error("Failed to convert received blob from backend:", error);
        }
  ...
  return (
    ...
    >
      <div ref={terminalRef} className="flex-1 w-full h-full bg-black"></div>
    </Popup>
  );
};
