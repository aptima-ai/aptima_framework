...
pub async fn install_solver_results_in_app_folder(
    ...
    for solver_result in solver_results {
        ...
        let base_dir = match solver_result.basic_info.type_and_name.pkg_type {
            PkgType::Extension => {
                app_dir.join(axis_PACKAGES_DIR).join(EXTENSION_DIR)
            }
            PkgType::Protocol => {
                app_dir.join(axis_PACKAGES_DIR).join(PROTOCOL_DIR)
            }
            PkgType::System => app_dir.join(axis_PACKAGES_DIR).join(SYSTEM_DIR),
            PkgType::AddonLoader => {
                app_dir.join(axis_PACKAGES_DIR).join(ADDON_LOADER_DIR)
            }
