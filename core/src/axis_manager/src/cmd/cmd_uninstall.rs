...
use axis_rust::pkg_info::constants::axis_PACKAGES_DIR;

use crate::{
    config::TmanConfig,
    constants::{
        DOT_axis_DIR, INSTALLED_PATHS_JSON_FILENAME, INSTALL_PATHS_APP_PREFIX,
        PACKAGE_INFO_DIR_IN_DOT_axis_DIR,
    },
...
async fn remove_installed_paths(
    ...
    let installed_paths_path = addon_path
        .join(DOT_axis_DIR)
        .join(PACKAGE_INFO_DIR_IN_DOT_axis_DIR)
    ...
    // Remove the .aptima/ directory.
    let axis_path = installed_paths_path.parent().unwrap().parent().unwrap();
    fs::remove_dir_all(axis_path)?;
