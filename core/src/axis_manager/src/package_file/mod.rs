...
use axis_rust::pkg_info::PkgInfo;
use axis_rust::pkg_info::{
    constants::axis_PACKAGES_DIR, manifest::parse_manifest_in_folder,
};

use super::{config::TmanConfig, constants::axis_PACKAGE_FILE_EXTENSION};
use crate::{
    constants::{
        DOT_axis_DIR, MANIFEST_JSON_FILENAME, PACKAGE_DIR_IN_DOT_axis_DIR,
    },
...
pub fn create_package_zip_file(
    ...
    let output_dir = folder_to_zip
        .join(DOT_axis_DIR)
        .join(PACKAGE_DIR_IN_DOT_axis_DIR);
    ...
    // is correct.
    axis_rust::pkg_info::property::check_property_json_of_pkg(
    ...
    // manifest.json is needed for all APTIMA packages.
    globset_builder.add(
    ...
    // Add exclude pattern for DOT_axis_DIR.
    overrides.add(&format!("!/{}/", DOT_axis_DIR))?;

    // Add exclude pattern for axis_PACKAGES_DIR.
    overrides.add(&format!("!/{}/", axis_PACKAGES_DIR))?;
