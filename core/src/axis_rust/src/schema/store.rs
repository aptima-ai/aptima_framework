...
pub fn are_axis_schemas_compatible(
...
pub fn are_cmd_schemas_compatible(
    ...
    let source = source.unwrap();
    let target = target.unwrap();
    are_axis_schemas_compatible(source.cmd.as_ref(), target.cmd.as_ref())?;
    are_axis_schemas_compatible(source.result.as_ref(), target.result.as_ref())?;
