...
use super::{
    constants::{PROPERTY_JSON_FILENAME, axis_FIELD_IN_PROPERTY},
    ...
    pub fn dump_property_to_file(
        ...
        if let Some(_ten) = &self._ten {
            property_json.insert(
                axis_FIELD_IN_PROPERTY.to_string(),
...
pub fn check_property_json_of_pkg(pkg_dir: &str) -> Result<()> {
    ...
    json_schema::axis_validate_property_json_file(
...
pub fn parse_property_in_folder(
    ...
    json_schema::axis_validate_property_json_file(&property_path).with_context(
...
mod tests {
    ...
    fn test_parse_property_with_known_fields() {
        ...
        let axis_in_property = property._ten.unwrap();

        assert_eq!(axis_in_property.uri.unwrap(), "http://example.com");
        assert!(axis_in_property.predefined_graphs.is_some());
        assert!(axis_in_property.additional_fields.is_empty());
    ...
    fn test_dump_property_to_file() {
        ...
        let axis_in_property = TenInProperty {
