...
pub fn axis_validate_manifest_json_string(data: &str) -> Result<()> {
...
pub fn axis_validate_manifest_json_file(file_path: &str) -> Result<()> {
...
pub fn axis_validate_property_json_string(data: &str) -> Result<()> {
...
pub fn axis_validate_property_json_file<P: AsRef<Path>>(
    file_path: P,
) -> Result<()> {
    let file = std::fs::File::open(file_path)?;
    let reader = std::io::BufReader::new(file);
...
mod tests {
    ...
    fn test_validate_default_cpp_app() {
        ...
        "#;
        let result = axis_validate_manifest_json_string(manifest);
        assert!(result.is_ok());
    }
    ...
    fn test_validate_invalid_graph_invalid_additional_property() {
        ...
        let result = axis_validate_property_json_string(property);
        assert!(result.is_err());
