...
PyObject *ten_py_ten_env_tester_send_video_frame(PyObject *self,
  ...
  if (cb_func) {
    ...
    success = ten_env_tester_send_video_frame(
        py_ten_env_tester->c_ten_env_tester, py_video_frame->msg.c_msg,
        proxy_send_xxx_callback, cb_func, &err);
  } else {
    success = ten_env_tester_send_video_frame(
        py_ten_env_tester->c_ten_env_tester, py_video_frame->msg.c_msg, NULL,
        NULL, &err);
  }
