...
static PyObject *ten_py_app_create(PyTypeObject *type, PyObject *args,
  ...
  if (!py_app->c_app) {
    TEN_ASSERT(0, "Failed to create APTIMA app.");

    Py_DECREF(py_app);
    return ten_py_raise_py_system_error_exception("Failed to create APTIMA app.");
  }
...
static PyObject *ten_py_app_close(PyObject *self, PyObject *args) {
  ...
  if (!rc) {
    return ten_py_raise_py_runtime_error_exception("Failed to close APTIMA app.");
  }
