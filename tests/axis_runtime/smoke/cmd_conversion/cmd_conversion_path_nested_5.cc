  ...
  void on_cmd(ten::axis_env_t &axis_env,
    ...
    if (cmd->get_name() == "hello_mapping") {
      ...
      if (json["test_property_name"] == 32) {
        auto result = ten::cmd_result_t::create(axis_STATUS_CODE_OK);
        result->set_property("detail", "hello world, too");
        axis_env.return_result(std::move(result), std::move(cmd));
      }
    }
  }
};
...
axis_CPP_REGISTER_ADDON_AS_EXTENSION(cmd_mapping_path_nested_5__test_extension_1,
                                    test_extension_1);
axis_CPP_REGISTER_ADDON_AS_EXTENSION(cmd_mapping_path_nested_5__test_extension_2,
                                    test_extension_2);
