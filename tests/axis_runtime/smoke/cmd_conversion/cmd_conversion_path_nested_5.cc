...
#include "include_internal/aptima_runtime/binding/cpp/detail/aptima_env_internal_accessor.h"
  ...
  void on_cmd(aptima::aptima_env_t &aptima_env,
    ...
    if (cmd->get_name() == "hello_mapping") {
      ...
      if (json["test_property_name"] == 32) {
        auto result = aptima::cmd_result_t::create(aptima_STATUS_CODE_OK);
        result->set_property("detail", "hello world, too");
        aptima_env.return_result(std::move(result), std::move(cmd));
      }
    }
  }
};
...
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(cmd_mapping_path_nested_5__test_extension_1,
                                    test_extension_1);
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(cmd_mapping_path_nested_5__test_extension_2,
                                    test_extension_2);
...
TEST(CmdConversionTest, CmdConversionPathNested5) {  // NOLINT
  ...
  hello_world_cmd->set_dest("msgpack://127.0.0.1:8001/", "default",
                            "cmd_mapping_path_nested_5__extension_group",
                            "test_extension_1");
