...
class test_extension_2 : public aptima::extension_t {
 public:
  ...
  void on_audio_frame(
    ...
    auto cmd = aptima::cmd_t::create("audio_frame_ack");
    if (audio_frame->get_bytes_per_sample() == 3 &&
        audio_frame->get_number_of_channels() == 555 &&
        audio_frame->get_data_fmt() == aptima_AUDIO_FRAME_DATA_FMT_INTERLEAVE) {
...
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(audio_frame_from_json__test_extension_1,
                                    test_extension_1);
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(audio_frame_from_json__test_extension_2,
                                    test_extension_2);
