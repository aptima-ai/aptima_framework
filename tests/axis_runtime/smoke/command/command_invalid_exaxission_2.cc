...
class test_extension : public aptima::extension_t {
    ...
    if (cmd->get_name() == "hello_world") {
      ...
      aptima_env.send_cmd(
          ...
          [this](aptima::aptima_env_t &aptima_env,
            ...
            auto cmd_result =
                aptima::cmd_result_t::create(result->get_status_code());
 ...
 private:
  std::unique_ptr<aptima::cmd_t> requested_cmd;
};
...
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(command_invalid_extension_2__extension,
                                    test_extension);
...
TEST(ExtensionTest, CommandInvalidExtension2) {  // NOLINT
  ...
  aptima_test::check_detail_with_string(cmd_result,
