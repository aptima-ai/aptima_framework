...
class test_extension_2 : public ten::extension_t {
  ...
  void on_configure(ten::axis_env_t &axis_env) override {
    bool rc = axis_env.init_property_from_json(
                           ...
                           "{\
                              ...
                              " axis_XSTR(EXTENSION_PROP_VALUE_GREETING) "\
                            }"
                           // clang-format on
    , nullptr);
    ASSERT_EQ(rc, true);
  ...
  void on_init(ten::axis_env_t &axis_env) override {
    ...
    fetch_property_thread_ = std::thread(
        [this](ten::axis_env_proxy_t *axis_env_proxy) {
          ...
          axis_env_proxy->notify(
              [this](ten::axis_env_t &axis_env) { this->get_property(axis_env); });
 ...
 private:
  ...
  void get_property(ten::axis_env_t &axis_env) {
    greeting_ = axis_env.get_property_string(EXTENSION_PROP_NAME_GREETING);
...
TEST(BasicTest, ExtensionsInitDependency) {  // NOLINT
  ...
  axis_test::check_status_code(cmd_result, axis_STATUS_CODE_OK);
  axis_test::check_detail_with_string(
      cmd_result,
