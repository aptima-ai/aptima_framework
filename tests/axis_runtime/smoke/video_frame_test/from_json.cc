 ...
 public:
  ...
  void on_cmd(ten::axis_env_t &axis_env,
    ...
    if (cmd->get_name() == "hello_world") {
      ...
      video_frame->set_pixel_fmt(axis_PIXEL_FMT_RGB24);
      video_frame->set_timestamp(12341234);

      axis_env.send_video_frame(std::move(video_frame));
    } else if (cmd->get_name() == "video_frame_ack") {
      auto cmd_result = ten::cmd_result_t::create(axis_STATUS_CODE_OK);
      cmd_result->set_property("detail", "hello world, too");
      axis_env.return_result(std::move(cmd_result), std::move(hello_world_cmd));
    }
  }
 ...
 public:
  ...
  void on_video_frame(
    ...
    if (video_frame->get_width() == 345 &&
        video_frame->get_pixel_fmt() == axis_PIXEL_FMT_RGB24) {
...
axis_CPP_REGISTER_ADDON_AS_EXTENSION(video_frame_from_json__test_extension_1,
                                    test_extension_1);
axis_CPP_REGISTER_ADDON_AS_EXTENSION(video_frame_from_json__test_extension_2,
                                    test_extension_2);
