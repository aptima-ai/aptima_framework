...
class extension_tester_1 : public ten::extension_tester_t {
 public:
  void set_on_started_callback(
      std::function<void(ten::aptima_env_tester_t &aptima_env)> callback) {
    on_started_callback_ = std::move(callback);
  }

  void set_on_hello_world_callback(
      std::function<void(ten::aptima_env_tester_t &aptima_env,
 ...
 private:
  std::function<void(ten::aptima_env_tester_t &aptima_env)> on_started_callback_;
  std::function<void(ten::aptima_env_tester_t &aptima_env,
                     std::unique_ptr<ten::cmd_t> cmd)>
...
typedef struct tester_context_t {
  ...
  ten::aptima_env_tester_proxy_t *aptima_env_proxy{nullptr};
} tester_context_t;
...
TEST(StandaloneTest, BasicGraphOuterThread2) {  // NOLINT
  ...
  std::thread tester_thread([&tester_context]() {
    ...
    tester->set_on_started_callback(
        [&tester_context](ten::aptima_env_tester_t &aptima_env) {
          std::lock_guard<std::mutex> lock(tester_context.mtx);
          tester_context.aptima_env_proxy =
              ten::aptima_env_tester_proxy_t::create(aptima_env);
