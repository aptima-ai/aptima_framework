 ...
 public:
  ...
  void on_configure(aptima::aptima_env_t &aptima_env) override {
    bool rc = aptima::aptima_env_internal_accessor_t::init_manifest_from_json(
        aptima_env, "invalid json string");
    ASSERT_EQ(rc, false);

    rc = aptima::aptima_env_internal_accessor_t::init_manifest_from_json(
        aptima_env,
        // clang-format off
                 R"({
                      "type": "extension",
...
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(schema_on_cmd__test_extension_1,
                                    test_extension_1);
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(schema_on_cmd__test_extension_2,
                                    test_extension_2);
