 ...
 public:
  ...
  void on_cmd(aptima::aptima_env_t &aptima_env,
    ...
    if (cmd->get_name() == "hello_world") {
      auto number_a = aptima_env.get_property_int8("a");
      ASSERT_EQ(static_cast<int8_t>(1), number_a);

      auto number_b = aptima_env.get_property_int16("b");
      ASSERT_EQ(static_cast<int16_t>(1), number_b);

      auto number_c = aptima_env.get_property_int32("c");
      ASSERT_EQ(static_cast<int32_t>(1), number_c);

      auto number_d = aptima_env.get_property_int64("d");
      ASSERT_EQ(static_cast<int64_t>(1), number_d);

      auto number_e = aptima_env.get_property_uint8("e");
      ASSERT_EQ(static_cast<uint8_t>(1), number_e);

      auto number_f = aptima_env.get_property_uint16("f");
      ASSERT_EQ(static_cast<uint16_t>(1), number_f);

      auto number_g = aptima_env.get_property_uint32("g");
      ASSERT_EQ(static_cast<uint32_t>(1), number_g);

      auto number_h = aptima_env.get_property_uint64("h");
      ASSERT_EQ(static_cast<uint64_t>(1), number_h);

      auto number_i = aptima_env.get_property_float32("i");
      ASSERT_EQ(true, fabs(1.0 - number_i) < 0.01);

      auto number_j = aptima_env.get_property_float64("j");
      ASSERT_EQ(true, fabs(1.0 - number_j) < 0.01);

      // The type of `hello` should be string, the `set_property` should be
      // failed.

      bool rc = aptima_env.set_property("hello", 1);
      ASSERT_EQ(rc, false);

      auto prop = aptima_env.get_property_string("hello");
      ASSERT_EQ("default", prop);

      aptima_env.set_property("hello", "world");
      prop = aptima_env.get_property_string("hello");
...
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(schema_set_extension_prop__test_extension,
                                    test_extension);
...
TEST(SchemaTest, SetExtensionProperty) {  // NOLINT
  ...
  aptima_test::check_status_code(cmd_result, aptima_STATUS_CODE_OK);
  aptima_test::check_detail_with_string(cmd_result, "world");
