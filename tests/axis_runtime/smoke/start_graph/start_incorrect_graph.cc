  ...
  void on_start(ten::aptima_env_t &aptima_env) override {
    ...
    aptima_env.send_cmd(
        ...
        [this](ten::aptima_env_t &aptima_env, std::unique_ptr<ten::cmd_result_t> cmd,
          ...
          EXPECT_EQ(status_code, aptima_STATUS_CODE_ERROR);
          ...
          if (test_cmd != nullptr) {
            ...
            aptima_env.return_result(std::move(cmd_result), std::move(test_cmd));
          }
        });
...
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(start_incorrect_graph__normal_extension,
