 ...
 public:
  ...
  void on_configure(ten::axis_env_t &axis_env) override {
    ...
    rc = axis_env.init_property_from_json(
                  "{\
          \"" EXTENSION_PROP_NAME_INT64 "\": " axis_XSTR(EXTENSION_PROP_VAL_INT64) ",\
          \"" EXTENSION_PROP_NAME_BOOL "\": " axis_XSTR(EXTENSION_PROP_VAL_BOOL) "}");
    ASSERT_EQ(rc, true);
  ...
  void on_cmd(ten::axis_env_t &axis_env,
    ...
    if (cmd->get_name() == "hello_world") {
      auto conn_property_value = axis_env.get_property_float64(CONN_PROP_NAME);

      auto extension_property_value_int64 =
          axis_env.get_property_int64(EXTENSION_PROP_NAME_INT64);

      auto extension_property_value_bool =
          axis_env.get_property_bool(EXTENSION_PROP_NAME_BOOL);
  ...
  void on_configure(ten::axis_env_t &axis_env) override {
    ...
    rc = axis_env.init_property_from_json(
        "{\
                    ...
                    \"" APP_PROP_NAME "\": " axis_XSTR(APP_PROP_VAL) "}");
    ASSERT_EQ(rc, true);
...
axis_CPP_REGISTER_ADDON_AS_EXTENSION(property_all__extension, test_extension);
