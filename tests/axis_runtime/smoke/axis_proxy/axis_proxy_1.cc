...
class test_extension_1 : public aptima::extension_t {
 public:
  ...
  void outer_thread_main(aptima::aptima_env_proxy_t *aptima_env_proxy) {  // NOLINT
    ...
    // Wait 2 seconds.
    aptima_LOGD("Waiting 2 seconds...");
    aptima_sleep(2000);

    // Before deleting aptima_env_proxy, this extension will not be removed, so
    // test_data can be accessed normally.
    EXPECT_EQ(*test_data, TEST_DATA_VALUE);

    // After this line, the whole APTIMA world will be closed.
  ...
  void on_start(aptima::aptima_env_t &aptima_env) override {
    ...
    // Create a aptima_env_proxy, and this will prevent APTIMA runtime from closing.
...
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(aptima_proxy_1__test_extension_1,
                                    test_extension_1);
...
TEST(ExtensionTest, TenEnvProxy1) {  // NOLINT
  ...
  start_graph_cmd->set_graph_from_json(R"({
           "nodes": [{
               ...
               "addon": "aptima_proxy_1__test_extension_1",
