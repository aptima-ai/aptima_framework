...
void extension_on_notify(aptima::aptima_env_t &aptima_env);
...
class test_extension : public aptima::extension_t {
 public:
  ...
  void outer_thread_main(aptima::aptima_env_proxy_t *aptima_env_proxy) {
    while (true) {
      if (trigger) {
        aptima_env_proxy->notify(extension_on_notify);
 ...
 private:
  friend void extension_on_notify(aptima::aptima_env_t &aptima_env);
...
void extension_on_notify(aptima::aptima_env_t &aptima_env) {
  auto *ext = static_cast<test_extension *>(
      aptima::aptima_env_internal_accessor_t::get_attached_target(aptima_env));
  auto cmd_result = aptima::cmd_result_t::create(aptima_STATUS_CODE_OK);
  cmd_result->set_property("detail", "hello world, too");
  aptima_env.return_result(std::move(cmd_result), std::move(ext->hello_world_cmd));
}
...
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(notify_test_normal_func__test_extension,
                                    test_extension);
