...
void extension_on_notify(ten::axis_env_t &axis_env);
...
class test_extension : public ten::extension_t {
 public:
  ...
  void outer_thread_main(ten::axis_env_proxy_t *axis_env_proxy) {
    while (true) {
      if (trigger) {
        axis_env_proxy->notify(extension_on_notify);
 ...
 private:
  friend void extension_on_notify(ten::axis_env_t &axis_env);
...
void extension_on_notify(ten::axis_env_t &axis_env) {
  auto *ext = static_cast<test_extension *>(
      ten::axis_env_internal_accessor_t::get_attached_target(axis_env));
  auto cmd_result = ten::cmd_result_t::create(axis_STATUS_CODE_OK);
  cmd_result->set_property("detail", "hello world, too");
  axis_env.return_result(std::move(cmd_result), std::move(ext->hello_world_cmd));
}
...
axis_CPP_REGISTER_ADDON_AS_EXTENSION(notify_test_normal_func__test_extension,
                                    test_extension);
