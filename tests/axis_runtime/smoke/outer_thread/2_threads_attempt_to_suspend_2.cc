...
class test_extension_1 : public aptima::extension_t {
 public:
  ...
  void outer_thread1_main(aptima::aptima_env_proxy_t *aptima_env_proxy) {  // NOLINT
    for (int i = 0; i < 10; ++i) {
      ...
      *test_data = TEST_DATA_VALUE;
      ...
      aptima_env_proxy->acquire_lock_mode();
      aptima_env_proxy->notify(send_data_from_outer_thread, test_data, true);
      aptima_env_proxy->release_lock_mode();
    }

    delete aptima_env_proxy;
  }

  void outer_thread2_main(aptima::aptima_env_proxy_t *aptima_env_proxy) {  // NOLINT
    for (int i = 0; i < 10; ++i) {
      // Create a memory buffer to contain some important data.
      auto *test_data = static_cast<int64_t *>(aptima_MALLOC(sizeof(int64_t)));
      aptima_ASSERT(test_data, "Failed to allocate memory.");
  ...
  void on_start(aptima::aptima_env_t &aptima_env) override {
    ...
    aptima_env.send_cmd(
        ...
        [this](aptima::aptima_env_t &aptima_env,
          ...
          auto *aptima_proxy_1 = aptima::aptima_env_proxy_t::create(aptima_env);
          auto *aptima_proxy_2 = aptima::aptima_env_proxy_t::create(aptima_env);
  ...
  void on_stop(aptima::aptima_env_t &aptima_env) override {
    ...
    aptima_LOGD("Join outer thread 1 start");
    outer_thread1->join();
    delete outer_thread1;
    aptima_LOGD("Join outer thread 1 end");

    aptima_LOGD("Join outer thread 2 start");
    outer_thread2->join();
    delete outer_thread2;
    aptima_LOGD("Join outer thread 2 end");
