  ...
  void on_cmd(aptima::aptima_env_t &aptima_env,
    ...
    if (cmd->get_name() == "hello_world") {
      aptima_env.send_cmd_ex(
          std::move(cmd), [this](aptima::aptima_env_t &aptima_env,
                                 std::unique_ptr<aptima::cmd_result_t> cmd_result,
                                 aptima::error_t *err) {
            ++received_result_cnt;

            if (received_result_cnt == 1) {
              aptima_ENV_LOG_INFO(aptima_env, "receives 1 cmd result");
            } else if (received_result_cnt == 2) {
              aptima_ENV_LOG_INFO(aptima_env, "receives 2 cmd result");
...
class test_extension_2 : public aptima::extension_t {
 public:
  ...
  void on_cmd(aptima::aptima_env_t &aptima_env,
              std::unique_ptr<aptima::cmd_t> cmd) override {
    if (cmd->get_name() == "hello_world") {
      auto cmd_result_1 = aptima::cmd_result_t::create(aptima_STATUS_CODE_OK);
      cmd_result_1->set_property("detail", "hello world, too");
      cmd_result_1->set_final(false);
      aptima_env.return_result(std::move(cmd_result_1), std::move(cmd));

      auto cmd_result_2 = aptima::cmd_result_t::create(aptima_STATUS_CODE_OK);
      cmd_result_2->set_property("detail", "hello world, too");
      aptima_env.return_result(std::move(cmd_result_2), std::move(cmd));
    }
  }
};
...
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(multiple_result_1__test_extension_1,
                                    test_extension_1);
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(multiple_result_1__test_extension_2,
                                    test_extension_2);
