    ...
    if (cmd->get_name() == "hello_world") {
      aptima_env.send_cmd(
          std::move(cmd), [this](aptima::aptima_env_t &aptima_env,
            ...
            aptima_ENV_LOG_INFO(aptima_env,
                             std::to_string(received_result_cnt).c_str());

            if (cmd_result->is_completed()) {
              aptima_ASSERT(received_result_cnt == 1, "Should not happen.");
...
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(multiple_result_2__test_extension_1,
                                    test_extension_1);
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(multiple_result_2__test_extension_2,
                                    test_extension_2);
aptima_CPP_REGISTER_ADDON_AS_EXTENSION(multiple_result_2__test_extension_3,
                                    test_extension_3);
