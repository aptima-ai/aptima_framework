...
def test_get_set_prop_python():
    ...
    if sys.platform == "win32":
        print("test_get_set_prop_python doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "get_set_prop_python_app/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    source_pkg_name = "get_set_prop_python_app"
    ...
    bootstrap_cmd = os.path.join(
        base_path, "get_set_prop_python_app/bin/bootstrap"
    )
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                "get_set_prop_python_app/aptima_packages/system/aptima_runtime/lib/libasan.so",
            )
    ...
    server_cmd = os.path.join(base_path, "get_set_prop_python_app/bin/start")
    ...
    if not is_started:
        print("The get_set_prop_python is not started after 10 seconds.")

        server.kill()
        exit_code = server.wait()
        print("The exit code of get_set_prop_python: ", exit_code)
    ...
    try:
        ...
        if not is_stopped:
            print("The get_set_prop_python can not stop after 30 seconds.")
            server.kill()
