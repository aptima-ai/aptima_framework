...
def test_two_async_exts_one_group_python():
    ...
    if sys.platform == "win32":
        print("test_two_async_exts_one_group_python doesn't support win32")
        assert False
    ...
    source_pkg_name = "two_async_exts_one_group_python_app"
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                "two_async_exts_one_group_python_app/aptima_packages/system/aptima_runtime/lib/libasan.so",
            )
