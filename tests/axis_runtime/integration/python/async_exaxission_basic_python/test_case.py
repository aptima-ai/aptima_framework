...
def test_async_extension_basic_python():
    ...
    source_pkg_name = "async_extension_basic_python_app"
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                "async_extension_basic_python_app/aptima_packages/system/aptima_runtime/lib/libasan.so",
            )
    ...
    if not is_started:
        print(
            "The async_extension_basic_python is not started after 10 seconds."
        )
