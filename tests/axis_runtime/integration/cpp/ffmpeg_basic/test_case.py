...
def test_ffmpeg_basic_app():
    ...
    if sys.platform == "win32":
        my_env["PATH"] = (
            os.path.join(
                base_path,
                "ffmpeg_basic_app/aptima_packages/system/aptima_runtime/lib",
            )
            + ";"
            + my_env["PATH"]
        )
        ...
        if (
            ...
            libasan_path = os.path.join(
                base_path,
                "ffmpeg_basic_app/aptima_packages/system/aptima_runtime/lib/libasan.so",
            )
    ...
    # APTIMA app of the subsequent integration test cases to use the 'libaptima_runtime.so'
