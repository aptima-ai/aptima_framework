...
def test_send_video_frame_nodejs():
    ...
    source_pkg_name = "send_video_frame_nodejs_app"
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                "send_video_frame_nodejs_app/aptima_packages/system/aptima_runtime/lib/libasan.so",
            )
    ...
    server_cmd = os.path.join(base_path, "send_video_frame_nodejs_app/bin/start")
    ...
    if not is_started:
        print("The send_video_frame_nodejs is not started after 10 seconds.")
