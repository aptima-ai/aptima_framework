...
TEST(PathTest, positive) {
  aptima::TenString str = ten_path_get_cwd();
  ...
  str = "/aaa/bbb/ccc.txt";
  aptima::TenString leaf = ten_path_get_filename(str);
  AGO_LOG("Leaf of %s is: %s\n", str.c_str(), leaf.c_str());
  aptima::TenString dir = ten_path_get_dirname(str);
  ...
  EXPECT_EQ(dir == "/", true);
  str = cwd + "/.";
  aptima::TenString abs = ten_path_realpath(str);
