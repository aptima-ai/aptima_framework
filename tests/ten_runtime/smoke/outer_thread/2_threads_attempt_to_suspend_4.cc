...
class test_extension_1 : public aptima::extension_t {
 public:
  ...
  void outer_thread1_main(aptima::ten_env_proxy_t *ten_env_proxy) {  // NOLINT
  ...
  void outer_thread2_main(aptima::ten_env_proxy_t *ten_env_proxy) {  // NOLINT
    // Create a memory buffer to contain some important data.
    auto *test_data = static_cast<int64_t *>(TEN_MALLOC(sizeof(int64_t)));
    TEN_ASSERT(test_data, "Failed to allocate memory.");
  ...
  void on_start(aptima::ten_env_t &ten_env) override {
    ...
    ten_env.send_cmd(
        ...
        [this](aptima::ten_env_t &ten_env,
          ...
          auto *ten_proxy_1 = aptima::ten_env_proxy_t::create(ten_env);
          auto *ten_proxy_2 = aptima::ten_env_proxy_t::create(ten_env);
