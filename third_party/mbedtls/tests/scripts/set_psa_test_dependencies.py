...
def dependencies_of_symbol(symbol):
    ...
    if symbol.startswith("PSA_ALG_CATEGORY_") or symbol.startswith(
        "PSA_KEY_TYPE_CATEGORY_"
    ):
...
def systematic_dependencies(file_name, function_name, arguments):
    ...
    if os.path.basename(
        file_name
    ) == "test_suite_psa_crypto_persistent_key.data" and function_name in {
...
def updated_dependencies(file_name, function_name, arguments, dependencies):
    ...
    manual = [
        ...
        if not (is_systematic_dependency(dep) or is_classic_dependency(dep))
    ]
...
def process_data_stanza(stanza, file_name, test_case_number):
    ...
    if len(content_matches) < 2:
        raise Exception(
            "Not enough content lines in paragraph {} in {}".format(
                test_case_number, file_name
            )
        )
    if len(content_matches) > 3:
        raise Exception(
            "Too many content lines in paragraph {} in {}".format(
                test_case_number, file_name
            )
        )
    ...
    new_dependencies = updated_dependencies(
        file_name, function_name, arguments, old_dependencies
    )
    if new_dependencies:
        stanza = (
            text_before + dependencies_leader + ":".join(new_dependencies) + text_after
        )
    else:
...
def process_data_file(file_name, old_content):
    ...
    new_stanzas = [
        process_data_stanza(stanza, file_name, n)
