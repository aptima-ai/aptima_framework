    ...
    def add_signature_algorithms(self, *signature_algorithms):
        self._sig_algs.extend(
            [
                ...
                if sig_alg not in self._sig_algs
            ]
        )
    ...
    def add_named_groups(self, *named_groups):
        self._named_groups.extend(
            [
                ...
                if named_group not in self._named_groups
            ]
        )
    ...
    def add_cert_signature_algorithms(self, *signature_algorithms):
        self._cert_sig_algs.extend(
            [
                sig_alg
                for sig_alg in signature_algorithms
                if sig_alg not in self._cert_sig_algs
            ]
        )
...
class OpenSSLBase(TLSProgram):
    ...
    def cmd(self):
        ...
        if self._named_groups:
            named_groups = ":".join(
                map(
                    lambda named_group: self.NAMED_GROUP[named_group],
                    self._named_groups,
                )
            )
...
class OpenSSLServ(OpenSSLBase):
    ...
    def pre_cmd(self):
        ...
        for _, cert, key in map(
            lambda sig_alg: CERTIFICATES[sig_alg], self._cert_sig_algs
        ):
...
class GnuTLSBase(TLSProgram):
    ...
    CIPHER_SUITE = {
        "TLS_AES_256_GCM_SHA384": ["AES-256-GCM", "SHA384", "AEAD"],
        "TLS_AES_128_GCM_SHA256": ["AES-128-GCM", "SHA256", "AEAD"],
        "TLS_CHACHA20_POLY1305_SHA256": ["CHACHA20-POLY1305", "SHA256", "AEAD"],
        "TLS_AES_128_CCM_SHA256": ["AES-128-CCM", "SHA256", "AEAD"],
        "TLS_AES_128_CCM_8_SHA256": ["AES-128-CCM-8", "SHA256", "AEAD"],
    }
    ...
    def cmd(self):
        ...
        if self._named_groups:
            priority_string_list.extend(
                update_priority_string_list(self._named_groups, self.NAMED_GROUP)
            )
        else:
            priority_string_list.append("GROUP-ALL")

        priority_string_list = ["NONE"] + priority_string_list + ["VERS-TLS1.3"]
        ...
        ret += ["--priority={priority_string}".format(priority_string=priority_string)]
        return ret
...
class GnuTLSServ(GnuTLSBase):
    ...
    def pre_cmd(self):
        ...
        for _, cert, key in map(
            ...
            ret += [
                "--x509certfile {cert} --x509keyfile {key}".format(cert=cert, key=key)
            ]
        return ret
...
class MbedTLSBase(TLSProgram):
    ...
    def pre_checks(self):
        ...
        if "rsa_pss_rsae_sha256" in self._sig_algs + self._cert_sig_algs:
            ret.append("requires_config_enabled MBEDTLS_X509_RSASSA_PSS_SUPPORT")
        return ret
...
class MbedTLSServ(MbedTLSBase):
    ...
    def post_checks(self):
        ...
        for named_group in self._named_groups:
            check_strings += [
                "got named group: {named_group}({iana_value:04x})".format(
...
class MbedTLSCli(MbedTLSBase):
    ...
    def hrr_post_checks(self, named_group):
        ...
        ret += [
            '-c "selected_group ( {:d} )"'.format(NAMED_GROUP_IANA_VALUE[named_group])
        ]
        return ret
    ...
    def post_checks(self):
        ...
        for named_group in self._named_groups:
            check_strings += [
                "NamedGroup: {named_group} ( {iana_value:x} )".format(
                    named_group=named_group,
                    iana_value=NAMED_GROUP_IANA_VALUE[named_group],
                )
            ]
...
def generate_compat_test(
    client=None, server=None, cipher=None, named_group=None, sig_alg=None
):
    """
    ...
    server_object = SERVER_CLASSES[server](
        ciphersuite=cipher,
        named_group=named_group,
        signature_algorithm=sig_alg,
        cert_sig_alg=sig_alg,
    )
    client_object = CLIENT_CLASSES[client](
        ciphersuite=cipher,
        named_group=named_group,
        signature_algorithm=sig_alg,
        cert_sig_alg=sig_alg,
...
def generate_hrr_compat_test(
    client=None,
    server=None,
    client_named_group=None,
    ...
    server_object = SERVER_CLASSES[server](
        named_group=server_named_group, cert_sig_alg=cert_sig_alg
    )

    client_object = CLIENT_CLASSES[client](
        named_group=client_named_group, cert_sig_alg=cert_sig_alg
    )
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Output file path if `-a` was set",
    )

    parser.add_argument(
        "-a",
        "--generate-all-tls13-compat-tests",
        action="store_true",
        default=False,
        help="Generate all available tls13 compat tests",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List supported ciphersuites",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List supported signature algorithms",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List supported named groups",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List supported TLS servers",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List supported TLS Clients",
    )
    ...
    parser.add_argument(
        "cipher",
        choices=CIPHER_SUITE_IANA_VALUE.keys(),
        nargs="?",
    ...
    parser.add_argument(
        "sig_alg",
        choices=SIG_ALG_IANA_VALUE.keys(),
        nargs="?",
    ...
    parser.add_argument(
        "named_group",
        choices=NAMED_GROUP_IANA_VALUE.keys(),
        nargs="?",
    ...
    def get_all_test_cases():
        ...
        for client, server, cipher, named_group, sig_alg in itertools.product(
            CLIENT_CLASSES.keys(),
            SERVER_CLASSES.keys(),
        ...
        for client, server, client_named_group, server_named_group in itertools.product(
            CLIENT_CLASSES.keys(),
            SERVER_CLASSES.keys(),
            NAMED_GROUP_IANA_VALUE.keys(),
            NAMED_GROUP_IANA_VALUE.keys(),
            ...
            if (
                client == "mbedTLS" or server == "mbedTLS"
            ) and client_named_group != server_named_group:
                yield generate_hrr_compat_test(
                    client=client,
                    server=server,
    ...
    if (
        args.list_ciphers
        or args.list_sig_algs
        or args.list_named_groups
        or args.list_servers
        or args.list_clients
    ):
