...
def gather_inputs(
    headers: Iterable[str], test_suites: Iterable[str], inputs_class=InputsForTest
) -> PSAMacroEnumerator:
...
ALG_TRUNCATED_TO_SELF_RE = re.compile(
    r"PSA_ALG_AEAD_WITH_SHORTENED_TAG\("
...
def collect_values(
    ...
    expressions = sorted(
        expr for expr in inputs.generate_expressions(names) if not is_simplifiable(expr)
    )
...
class Tests:
    ...
    def run_one(self, inputs: InputsForTest, type_word: str) -> None:
        ...
        output_bytes = subprocess.check_output(
            [self.options.program, type_word] + values
        )
...
def main():
    ...
    parser.add_argument(
        "--show", action="store_true", help="Show tested values on stdout"
    )
    parser.add_argument(
        "--no-show",
        action="store_false",
