...
_implemented_dependencies = (
    None
)  # type: Optional[FrozenSet[str]] #pylint: disable=invalid-name


def hack_dependencies_not_implemented(dependencies: List[str]) -> None:
    global _implemented_dependencies  # pylint: disable=global-statement,invalid-name
    if _implemented_dependencies is None:
        _implemented_dependencies = read_implemented_dependencies(
            "include/psa/crypto_config.h"
        )
    if not all(
        (dep.lstrip("!") in _implemented_dependencies or "PSA_WANT" not in dep)
...
def test_case_for_key_type_not_supported(
    ...
    *args: str,
    ...
    tc.set_description(
        "PSA {} {} {}-bit {} supported".format(verb, short_key_type, bits, adverb)
    )
...
class NotSupported:
    ...
    ALWAYS_SUPPORTED = frozenset(
        ["PSA_KEY_TYPE_DERIVE", "PSA_KEY_TYPE_RAW_DATA", "PSA_KEY_TYPE_HMAC"]
    )
        ...
        if alg.can_do(category):
            ...
            for dep in automatic_dependencies(alg.base_expression):
                yield self.make_test_case(
                    alg, category, self.Reason.NOT_SUPPORTED, not_deps=frozenset([dep])
                )
        else:
    ...
    def one_key_test_cases(
        ...
        for kt in self.key_types:
            ...
            if key_is_compatible and alg.can_do(category):
                ...
                yield self.make_test_case(
                    alg, category, self.Reason.INCOMPATIBLE, kt=kt
                )
            else:
...
class StorageTestData(StorageKey):
    ...
    def __init__(
        self, description: str, expected_usage: Optional[List[str]] = None, **kwargs
    ) -> None:
...
class StorageFormat:
    ...
    def __init__(self, info: Information, version: int, forward: bool) -> None:
        ...
        * `forward`: if true, generate forward compatibility test cases which
        ...
        self.constructors = (
            info.constructors
        )  # type: macro_collector.PSAMacroEnumerator
    ...
    def exercise_key_with_algorithm(
        cls, key_type: psa_storage.Expr, bits: int, alg: psa_storage.Expr
    ) -> bool:
...
class StorageFormatV0(StorageFormat):
    ...
    def gather_key_types_for_sign_alg(self) -> Dict[str, List[str]]:
        ...
        key_types = set(
            self.constructors.generate_expressions(self.constructors.key_types)
        )
        algorithms = set(
            self.constructors.generate_expressions(self.constructors.sign_algorithms)
        )
        ...
        for alg in algorithms:
            ...
            for key_type in key_types:
                ...
                key_type_keywords = set(
                    key_type.translate(translation_table).split(sep="_")[3:]
                )
                ...
                if key_type_keywords.isdisjoint(
                    incompatible_key_type_keywords
                ) and not key_type_keywords.isdisjoint(alg_keywords):
...
class TestGenerator:
    ...
    def write_test_data_file(
        self, basename: str, test_cases: Iterable[test_case.TestCase]
    ) -> None:
    ...
    TARGETS = {
        "test_suite_psa_crypto_generate_key.generated": lambda info: KeyGenerate(
            info
        ).test_cases_for_key_generation(),
        "test_suite_psa_crypto_not_supported.generated": lambda info: NotSupported(
            info
        ).test_cases_for_not_supported(),
        "test_suite_psa_crypto_op_fail.generated": lambda info: OpFail(
            info
        ).all_test_cases(),
        "test_suite_psa_crypto_storage_format.current": lambda info: StorageFormatForward(
            info, 0
        ).all_test_cases(),
        "test_suite_psa_crypto_storage_format.v0": lambda info: StorageFormatV0(
            info
        ).all_test_cases(),
...
def main(args):
    ...
    parser.add_argument(
        "--directory", metavar="DIR", help="Output directory (default: tests/suites)"
    )
    parser.add_argument(
        "targets",
        nargs="*",
    ...
    if options.list_for_cmake:
        print(
            ";".join(
                generator.filename_for(name) for name in sorted(generator.TARGETS)
            ),
            end="",
        )
        return
    if options.targets:
        ...
        options.targets = [
            os.path.basename(re.sub(r"\.data\Z", r"", target))
