...
class Key:
    ...
    def pack(
        ...
        * Only integer-valued elements are supported.
        """
        return struct.pack(
            "<" + fmt,  # little-endian, standard sizes
...
class TestKey(unittest.TestCase):
    ...
    def test_names(self):
        ...
        expected_hex = (
            "505341004b45590000000000010000000110f8ff000000000000000000000000ff1f0000"
            + "00" * length
        )
        self.assertEqual(key.bytes(), bytes.fromhex(expected_hex))
        self.assertEqual(key.hex(), expected_hex)
    ...
    def test_defaults(self):
        ...
        expected_hex = (
            "505341004b455900000000000100000001100800000000000000000000000000010000002a"
        )
        self.assertEqual(key.bytes(), bytes.fromhex(expected_hex))
        self.assertEqual(key.hex(), expected_hex)
