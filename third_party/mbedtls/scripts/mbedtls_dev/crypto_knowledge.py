...
class KeyType:
    ...
    KEY_TYPE_SIZES = {
        ...
        "PSA_KEY_TYPE_HMAC": (
            ...
            384,
            512,
        ),  # standard size for each supported hash
    ...
    def key_material(self, bits: int) -> bytes:
        ...
        if self.expression in ASYMMETRIC_KEY_DATA:
            if bits not in ASYMMETRIC_KEY_DATA[self.expression]:
                raise ValueError(
                    "No key data for {}-bit {}".format(bits, self.expression)
                )
            return ASYMMETRIC_KEY_DATA[self.expression][bits]
        if bits % 8 != 0:
            raise ValueError(
                "Non-integer number of bytes: {} bits for {}".format(
                    bits, self.expression
                )
            )
        length = bits // 8
    ...
    def can_do(self, alg: "Algorithm") -> bool:
        ...
        if self.head == "ECC":
            ...
            if alg.head == "ECDSA" and eccc == EllipticCurveCategory.SHORT_WEIERSTRASS:
                return True
            if (
                alg.head in {"PURE_EDDSA", "EDDSA_PREHASH"}
                and eccc == EllipticCurveCategory.TWISTED_EDWARDS
            ):
                return True
        return False
...
class Algorithm:
    ...
    def can_do(self, category: AlgorithmCategory) -> bool:
        ...
        if (
            ...
            and self.is_key_agreement_with_derivation()
        ):
            return True
        return False
    ...
    def usage_flags(self, public: bool = False) -> List[str]:
        ...
        if self.category == AlgorithmCategory.HASH:
            ...
            self.category == AlgorithmCategory.CIPHER
            or self.category == AlgorithmCategory.AEAD
        ):
            ...
            self.category == AlgorithmCategory.KEY_DERIVATION
            or self.category == AlgorithmCategory.KEY_AGREEMENT
        ):
            flags = ["DERIVE"]
        else:
