...
class TestProxy:
    ...
    @pytest.mark.skipif(condition=not Env.have_nghttpx(), reason="no nghttpx available")
    def test_10_02_proxys_up(
        self, env: Env, httpd, nghttpx, proto, fname, fcount, repeat
    ):
        if proto == "h2" and not env.curl_uses_lib("nghttp2"):
            pytest.skip("only supported with nghttp2")
        count = fcount
    ...
    @pytest.mark.skipif(condition=not Env.have_nghttpx(), reason="no nghttpx available")
    def test_10_06_proxytunnel_https(
        self, env: Env, httpd, nghttpx_fwd, proto, tunnel, repeat
    ):
    ...
    def test_10_07_pts_down_small(
        self, env: Env, httpd, nghttpx_fwd, proto, tunnel, fname, fcount, repeat
    ):
        if tunnel == "h2" and not env.curl_uses_lib("nghttp2"):
            pytest.skip("only supported with nghttp2")
        count = fcount
    ...
    def test_10_08_upload_seq_large(
        self, env: Env, httpd, nghttpx, proto, tunnel, fname, fcount, repeat
    ):
        if tunnel == "h2" and not env.curl_uses_lib("nghttp2"):
            pytest.skip("only supported with nghttp2")
        count = fcount
