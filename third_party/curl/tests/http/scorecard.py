...
class ScoreCard:
    ...
    def handshakes(self, proto: str) -> Dict[str, Any]:
        ...
        for authority in [f"{self.env.authority_for(self.env.domain1, proto)}"]:
            self.info("  localhost...")
            c_samples = []
            hs_samples = []
            errors = []
    ...
    def transfer_single(self, url: str, proto: str, count: int):
        ...
        for i in range(sample_size):
            ...
            r = curl.http_download(
                urls=[url], alpn_proto=proto, no_save=True, with_headers=False
            )
    ...
    def downloads(self, proto: str, count: int, fsizes: List[int]) -> Dict[str, Any]:
        scores = {}
        if self.httpd:
            if proto == "h3":
            ...
            for fsize in fsizes:
                label = (
                    f"{int(fsize / 1024)}KB"
                    if fsize < 1024 * 1024
                    else f"{int(fsize / (1024 * 1024))}MB"
                )
                fname = f"score{label}.data"
                self._make_docs_file(
                    docs_dir=self.httpd.docs_dir, fname=fname, fsize=fsize
                )
                url = f"https://{self.env.domain1}:{port}/{fname}"
                results = self.download_url(
                    label=label, url=url, proto=proto, count=count
                )
        ...
        if self.caddy:
            ...
            for fsize in fsizes:
                ...
                fname = f"score{label}.data"
                self._make_docs_file(
                    docs_dir=self.caddy.docs_dir, fname=fname, fsize=fsize
                )
                url = f"https://{self.env.domain1}:{port}/{fname}"
                results = self.download_url(
                    label=label, url=url, proto=proto, count=count
    ...
    def requests_url(self, url: str, proto: str, count: int):
        ...
        props = {
            ...
            "par-6": self.do_requests(
                url=url, proto=proto, count=count, max_parallel=6
            ),
            "par-25": self.do_requests(
                url=url, proto=proto, count=count, max_parallel=25
            ),
            "par-50": self.do_requests(
                url=url, proto=proto, count=count, max_parallel=50
            ),
            "par-100": self.do_requests(
                url=url, proto=proto, count=count, max_parallel=100
            ),
        }
        self.info(f"ok.\n")
        return props
    ...
    def requests(self, proto: str) -> Dict[str, Any]:
        ...
        if self.httpd:
            ...
            self._make_docs_file(
                docs_dir=self.httpd.docs_dir, fname="reqs10.data", fsize=10 * 1024
            )
        ...
        if self.caddy:
            ...
            self._make_docs_file(
                docs_dir=self.caddy.docs_dir, fname="req10.data", fsize=10 * 1024
            )
    ...
    def score_proto(
        ...
        if proto == "h3":
            ...
            p["name"] = proto
            p["implementation"] = (
                "hyper" if self.env.curl_uses_lib("hyper") else "native"
            )
        else:
        ...
        if downloads and len(downloads) > 0:
            score["downloads"] = self.downloads(
                proto=proto, count=download_count, fsizes=downloads
            )
        if requests:
    ...
    def print_score(self, score):
        ...
        if "downloads" in score:
            print("Downloads")
            print(
                f'  {"Server":<8} {"Size":>8} {"Single":>12} {"Serial":>12}'
