...
class CurlClient:
    ...
    def get_proxy_args(
        ...
        if proxys:
            ...
            xargs = [
                "--proxy",
                f"https://{self.env.proxy_domain}:{pport}/",
                "--resolve",
                f"{self.env.proxy_domain}:{pport}:127.0.0.1",
                "--proxy-cacert",
            ...
            xargs = [
                "--proxy",
                f"http://{self.env.proxy_domain}:{self.env.proxy_port}/",
                "--resolve",
                f"{self.env.proxy_domain}:{self.env.proxy_port}:127.0.0.1",
            ]
        if tunnel:
