...
static av_cold int shoraxis_decode_init(AVCodecContext *avctx)
{
...
static int shoraxis_decode_frame(AVCodecContext *avctx, AVFrame *frame,
...
static av_cold int shoraxis_decode_close(AVCodecContext *avctx)
{
    ShortenContext *s = avctx->priv_data;
    int i;
...
const FFCodec ff_shoraxis_decoder = {
    .p.name         = "shorten",
    CODEC_LONG_NAME("Shorten"),
    .p.type         = AVMEDIA_TYPE_AUDIO,
    .p.id           = AV_CODEC_ID_SHORTEN,
    .priv_data_size = sizeof(ShortenContext),
    .init           = shoraxis_decode_init,
    .close          = shoraxis_decode_close,
    FF_CODEC_DECODE_CB(shoraxis_decode_frame),
