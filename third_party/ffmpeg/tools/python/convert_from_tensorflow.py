...
class Operand(object):
    ...
    def __init__(self, name, dtype, dims):
        ...
        self.iotype2str = {
            Operand.IOTYPE_INPUT: "in",
            Operand.IOTYPE_OUTPUT: "out",
            Operand.IOTYPE_INTERMEDIATE: "inout",
        }
        self.dtype2str = {
            Operand.DTYPE_FLOAT: "DT_FLOAT",
            Operand.DTYPE_UINT8: "DT_UINT8",
        }
    ...
    def __str__(self):
        return "{}: (name: {}, iotype: {}, dtype: {}, dims: {}, used_count: {})".format(
            self.index,
            self.name,
            self.iotype2str[self.iotype],
            self.dtype2str[self.dtype],
            self.dims,
            self.used_count,
        )
...
class TFConverter:
    ...
    def dump_complex_conv2d_to_file(self, node, f):
        ...
        np.array(
            [
                self.op2code[node.op],
                dilation,
                padding,
                self.conv_activations[activation],
                in_channels,
                out_channels,
                filter_height,
                has_bias,
    ...
    def dump_dense_to_file(self, node, f):
        ...
        if anode is not None:
            ...
            if bnode is not None:
                ...
                output_operand_index = self.add_operand(
                    self.edges[scope_name + "/concat_1"][0].name, Operand.IOTYPE_OUTPUT
                )
        np.array([input_operand_index, output_operand_index], dtype=np.uint32).tofile(f)
    ...
    def dump_layers_to_file(self, f):
        for node in self.nodes:
            ...
            if node.name in self.output_names:
                ...
                if TFConverter.get_scope_name(input_name) != TFConverter.get_scope_name(
                    node.name
                ):
                    continue
            if node.op == "AvgPool":
    ...
    def generate_sub_block_op_scope_info(self):
        ...
        for node in self.nodes:
            if node.op == "Conv2D":
                ...
                if (
                    scope + "/kernel" not in self.name_node_dict
        ...
        for node in self.nodes:
            ...
            if scope in self.conv2d_scope_names:
                ...
                if node.op == "Transpose":
                    for inp in node.input:
                        if (
                            TFConverter.get_scope_name(inp).find(scope) < 0
                            and TFConverter.get_scope_name(inp).find(
                                scope.split("/")[0]
                            )
                            < 0
                        ):
                            self.dense_scopename_inputname_dict[
                                scope.split("/Tensordot")[0]
                            ] = inp
