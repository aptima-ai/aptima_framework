...
def check_structure():
    ...
    required_sections = ["Examples", "Version history"]
    ...
    for file in files:
        with open(file) as file_content:
            ...
            in_initial_code_example = (
                False  # whether we are inside the first code example block
            )
            ...
            documented_overloads = (
                {}
            )  # the overloads that have been documented in the current block
            ...
            for lineno, original_line in enumerate(file_content.readlines()):
                ...
                if h1sections > 1:
                    report(
                        "structure/unexpected_section",
                        f"{file}:{lineno+1}",
                        f'unexpected top-level title "{line}"',
                    )
                    h1sections = 1
                ...
                if line == "# Overview":
                    report(
                        "style/title",
                        f"{file}:{lineno+1}",
                        'overview pages should have a better title than "Overview"',
                    )
                ...
                if len(line) > 160 and "|" not in line:
                    report(
                        ...
                        f"{file}:{lineno+1} ({current_section})",
                        f"line is too long ({len(line)} vs. 160 chars)",
                    )
                ...
                if line.startswith("## "):
                    ...
                    if current_section in documented_overloads and last_overload != 0:
                        if (
                            len(documented_overloads[current_section]) > 0
                            ...
                            undocumented = [
                                x
                                for x in expected
                                if x not in documented_overloads[current_section]
                            ]
                            unexpected = [
                                x
                                for x in documented_overloads[current_section]
                            ...
                            if len(undocumented):
                                report(
                                    ...
                                    f'undocumented overloads: {", ".join([f"({x})" for x in undocumented])}',
                                )
                            if len(unexpected):
                                report(
                                    "style/numbering",
                                    f"{file}:{lineno} ({current_section})",
                                    f'unexpected overloads: {", ".join([f"({x})" for x in unexpected])}',
                                )
                    ...
                    if current_section in expected_sections:
                        ...
                        if idx <= section_idx:
                            report(
                                ...
                                f"{file}:{lineno+1}",
                                f'section "{current_section}" is in an unexpected order (should be before "{expected_sections[section_idx]}")',
                            )
                        section_idx = idx
                    else:
                        if (
                            "index.md" not in file
                        ):  # index.md files may have a different structure
                            report(
                                "structure/unknown_section",
                                f"{file}:{lineno+1}",
                                f'section "{current_section}" is not part of the expected sections',
                            )
                ...
                if (
                    in_initial_code_example
                    and line.startswith("//")
                    and line not in ["// since C++20", "// until C++20"]
                ):
                    ...
                    if any(map(str.isdigit, line)):
                        ...
                        if number != last_overload + 1:
                            report(
                                "style/numbering",
                                f"{file}:{lineno+1}",
                                f"expected number ({number}) to be ({last_overload +1 })",
                            )
                    ...
                    if any(map(str.isdigit, line)) and "(" not in line:
                        report(
                            "style/numbering",
                            f"{file}:{lineno+1}",
                            f"number should be in parentheses: {line}",
                        )
                ...
                if line == "" and previous_line == "":
                    report(
                        ...
                        f"{file}:{lineno}-{lineno+1} ({current_section})",
                ...
                if untitled_admonition and untitled_admonition.group(2) != "example":
                    report(
                        ...
                        f"{file}:{lineno} ({current_section})",
                        f'"{untitled_admonition.group(2)}" admonitions should have a title',
                    )
                ...
                for required_section in required_sections:
                    if required_section not in existing_sections:
                        report(
                            ...
                            f"{file}:{lineno+1}",
                            f'required section "{required_section}" was not found',
                        )
    ...
    for example_file in example_files:
        ...
        if not found:
            report(
                "examples/missing",
                f"{example_file}",
                "example file is not used in any documentation file",
            )
