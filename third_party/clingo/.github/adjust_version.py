...
def adjust_version(url):
    ...
    post = 0
    if (
        search(r"{}-{}\.tar\.gz".format(package_regex, escape(version)), pip)
        is not None
    ):
        post = 1

    for m in finditer(
        r"{}-{}\.post([0-9]+)\.tar\.gz".format(package_regex, escape(version)), pip
    ):
    ...
    with open(setup_path, "w") as fw:
        if setup_type == "cfg":
            if post > 0:
                ...
                fw.write(
                    sub("version( *)=.*", "version = {}".format(version), setup, 1)
                )
        else:
            if post > 0:
                fw.write(
