...
versionParser = re.compile(
    r"(\s*static\sVersion\sversion)\s*\(\s*(.*)\s*,\s*(.*)\s*,\s*(.*)\s*,\s*\"(.*)\"\s*,\s*(.*)\s*\).*"
)
...
class Version:
    ...
    def getVersionString(self):
        versionString = "{0}.{1}.{2}".format(
            self.majorVersion, self.minorVersion, self.patchNumber
        )
        if self.branchName != "":
            versionString = versionString + "-{0}.{1}".format(
                self.branchName, self.buildNumber
            )
...
def updateCmakeFile(version):
    ...
    replacementRegex = re.compile(
        b"""VERSION (\\d+.\\d+.\\d+) # CML version placeholder, don't delete"""
    )
    replacement = """VERSION {0} # CML version placeholder, don't delete""".format(
...
def updateMesonFile(version):
    with open(mesonPath, "rb") as file:
        lines = file.readlines()
    replacementRegex = re.compile(
        b"""version\s*:\s*'(\\d+.\\d+.\\d+)', # CML version placeholder, don't delete"""
    )
    replacement = """version: '{0}', # CML version placeholder, don't delete""".format(
        version.getVersionString()
    ).encode("ascii")
...
def updateVersionDefine(version):
    ...
    replacementRegexes = [
        (
            ...
            "#define CATCH_VERSION_MAJOR {}".format(version.majorVersion).encode(
                "ascii"
            ),
        ),
        (
            re.compile(b"#define CATCH_VERSION_MINOR \\d+"),
            "#define CATCH_VERSION_MINOR {}".format(version.minorVersion).encode(
                "ascii"
            ),
        ),
        (
            re.compile(b"#define CATCH_VERSION_PATCH \\d+"),
            "#define CATCH_VERSION_PATCH {}".format(version.patchNumber).encode(
                "ascii"
            ),
        ),
    ]
...
def updateVersionPlaceholder(filename, version):
    ...
    replacement = "in Catch2 {}.{}.{}".format(
        version.majorVersion, version.minorVersion, version.patchNumber
    ).encode("ascii")
