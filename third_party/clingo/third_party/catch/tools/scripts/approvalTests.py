...
if len(sys.argv) == 3:
    ...
    print(
        "Usage: {} path-to-SelfTest-executable path-to-temp-output-dir".format(
            sys.argv[0]
        )
    )
    exit(1)
...
def diffFiles(fileA, fileB):
    ...
    diff = difflib.unified_diff(
        aLines, bLines, fromfile=shortenedFilenameA, tofile=shortenedFilenameB, n=0
    )
...
def check_outputs(baseName):
    ...
    filteredFile = io.open(
        filteredResultsPath, "w", encoding="utf-8", errors="surrogateescape"
    )
    for line in rawFile:
...
approve(
    "console.swa4",
    [
        "~[!nonportable]~[!benchmark]~[approvals] *",
        "-s",
        "-w",
        "NoAssertions",
        "-x",
