...
def configure_and_build(
    source_path: str, project_path: str, options: List[Tuple[str, str]]
):
    base_configure_cmd = [
        "cmake",
    ...
    try:
        ...
        print(
            "Could not configure build to '{}' from '{}'".format(
    ...
    try:
        subprocess.run(
            build_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=True
        )
    except subprocess.SubprocessError as ex:
...
def run_and_return_output(
    base_path: str, binary_name: str, other_options: List[str]
) -> Tuple[str, str]:
