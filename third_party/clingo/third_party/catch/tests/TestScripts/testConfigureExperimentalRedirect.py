...
build_dir_path = os.path.join(
    os.path.abspath(sys.argv[2]), "CMakeConfigTests", "ExperimentalRedirect"
)

configure_and_build(
    catch2_source_path, build_dir_path, [("CATCH_CONFIG_EXPERIMENTAL_REDIRECT", "ON")]
)

stdout, _ = run_and_return_output(
    os.path.join(build_dir_path, "tests"), "SelfTest", ["-r", "xml", '"has printf"']
)
