...
struct http_server_t {
  struct lws_context *lws_context{nullptr};
  aptima::ten_env_proxy_t *ten_env_proxy{nullptr};
...
void proceed_to_stop_http_extension(aptima::ten_env_t &ten_env,
    ...
    case LWS_CALLBACK_HTTP_DROP_PROTOCOL:
      ...
      if (http_server->all_http_session_data.empty() &&
        ...
        http_server->ten_env_proxy->notify(
            [http_server](aptima::ten_env_t &ten_env) {
...
class http_server_extension_t : public aptima::extension_t {
 public:
  ...
  void on_start(aptima::ten_env_t &ten_env) override {
    ...
    http_server->ten_env_proxy = aptima::ten_env_proxy_t::create(ten_env);
  ...
  void on_stop(aptima::ten_env_t & /*aptima*/) override {
    // Extension stop.
...
void send_ten_msg_with_req_body(
    const std::shared_ptr<http_transaction_data_t> &http_session_data) {
  // We are _not_ in the APTIMA threads, so we need to use ten_env_proxy.
  http_session_data->http_server->ten_env_proxy->notify(
      [http_session_data](aptima::ten_env_t &ten_env) {
        ...
        ten_env.send_cmd(
            ...
            [http_session_data](aptima::ten_env_t &ten_env,
              ...
              auto *ext = static_cast<http_server_extension_t *>(
                  aptima::ten_env_internal_accessor_t::get_attached_target(
                      ten_env));
