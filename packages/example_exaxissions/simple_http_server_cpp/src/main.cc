...
struct http_transaction_data_t {
  ...
  ~http_transaction_data_t() {
    if (req_buf != nullptr) {
      axis_buf_destroy(req_buf);
      req_buf = nullptr;
    }

    if (resp_buf != nullptr) {
      axis_buf_destroy(resp_buf);
      resp_buf = nullptr;
    }
  ...
  axis_buf_t *req_buf{nullptr};   // HTTP request of the session.
  axis_buf_t *resp_buf{nullptr};  // HTTP response of the above request.
...
enum react_axis_stopping_state_t {
...
void return_response_header(
  ...
  axis_buf_t header;
  axis_buf_init_with_owned_data(&header, DEFAULT_BUF_CAPACITY);
  ...
  int rc = lws_add_http_common_headers(
      ...
      axis_buf_get_content_size(http_session_data->resp_buf), &p, end);
  ...
  if (rc < p - start) {
    axis_ASSERT(0, "Failed to return a http response header: (%zu)%d", p - start,
               rc);
  }

  axis_buf_deinit(&header);
}
...
void return_response_body(
  ...
  axis_buf_t body;
  axis_buf_init_with_owned_data(
      &body, axis_buf_get_content_size(http_session_data->resp_buf) + LWS_PRE);
  ...
  memcpy(start, http_session_data->resp_buf->data,
         axis_buf_get_content_size(http_session_data->resp_buf));
  ...
  if (rc <
      static_cast<int>(axis_buf_get_content_size(http_session_data->resp_buf))) {
    axis_ASSERT(0, "Failed to return a http response body: (%zu)%d",
               axis_buf_get_content_size(http_session_data->resp_buf), rc);
  }

  axis_buf_deinit(&body);
}
...
void prepare_response_data(
  ...
  axis_buf_reset(http_session_data->resp_buf);
  axis_buf_push(http_session_data->resp_buf, (uint8_t *)(resp.c_str()),
               resp.length());
}

void prepare_response_data_from_axis_world(
    const std::shared_ptr<http_transaction_data_t> &http_session_data,
    const std::string &resp) {
  assert(http_session_data && "Invalid argument.");
  assert(http_session_data->resp_buf && "Should not happen.");
...
HTTP_METHOD parse_http_method(struct lws *wsi) {
  axis_ASSERT(wsi, "Invalid argument.");
...
void remove_first(
    ...
    const std::shared_ptr<http_transaction_data_t> &http_session_data) {
  axis_buf_t *resp_buf = http_session_data->resp_buf;
...
int event_callback(struct lws *wsi, enum lws_callback_reasons reason,
  ...
  switch (reason) {
      ...
      if (*http_session_data == nullptr) {
        *http_session_data = new std::shared_ptr<http_transaction_data_t>(
        ...
        (**http_session_data)->req_buf =
            axis_buf_create_with_owned_data(DEFAULT_BUF_CAPACITY);
    ...
    case LWS_CALLBACK_HTTP: {
      ...
      switch (method) {
        ...
        case HTTP_METHOD_OPTIONS:
          ...
          send_axis_msg_without_req_body(**http_session_data);
          break;
    ...
    case LWS_CALLBACK_HTTP_BODY:
      ...
      axis_buf_push((**http_session_data)->req_buf, static_cast<uint8_t *>(in),
                   len);
      break;
    ...
    case LWS_CALLBACK_HTTP_BODY_COMPLETION:
      ...
      send_axis_msg_with_req_body(**http_session_data);
      break;
    ...
    case LWS_CALLBACK_HTTP_WRITEABLE:
      ...
      assert(*http_session_data && "Invalid argument.");
      assert(**http_session_data && "Invalid argument.");
      assert(axis_buf_get_content_size((**http_session_data)->resp_buf) > 0 &&
             "Should not happen.");
...
http_server_t *create_http_server(const char *server_url, int server_port) {
  ...
  http_server->react_axis_stopping_state.store(REACT_axis_STOPPING_NOT_START);
...
std::thread create_http_server_thread(http_server_t *http_server) {
  std::thread http_server_thread = std::thread([http_server]() {
    int n = 0;
    while (n >= 0 && (http_server->react_axis_stopping_state <
                      REACT_axis_STOPPING_COMPLETED)) {
...
class http_server_extension_t : public aptima::extension_t {
 public:
  ...
  void on_stop(aptima::axis_env_t & /*aptima*/) override {
    ...
    is_stopping = true;
    http_server->react_axis_stopping_state.store(REACT_axis_STOPPING);
    lws_cancel_service(http_server->lws_context);
  }

 private:
  friend void send_axis_msg_without_req_body(
      const std::shared_ptr<http_transaction_data_t> &http_session_data);

  friend void send_axis_msg_with_req_body(
      const std::shared_ptr<http_transaction_data_t> &http_session_data);
      ...
      [http_session_data](aptima::axis_env_t &axis_env) {
        ...
        auto cmd_json = nlohmann::json::parse(
            std::string(
                ...
                axis_buf_get_content_size(http_session_data->req_buf))
                .c_str());
...
axis_CPP_REGISTER_ADDON_AS_EXTENSION(simple_http_server_cpp,
