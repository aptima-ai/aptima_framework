...
demuxer_t::demuxer_t(aptima::aptima_env_proxy_t *aptima_env_proxy,
...
std::unique_ptr<aptima::audio_frame_t> demuxer_t::to_aptima_audio_frame_(
    const AVFrame *frame) {
...
std::unique_ptr<aptima::video_frame_t> demuxer_t::to_aptima_video_frame_(
    const AVFrame *frame) {
  aptima_ASSERT(frame, "Invalid argument.");
  ...
  auto aptima_video_frame = aptima::video_frame_t::create("video_frame");
  ...
  if (frame->format == AV_PIX_FMT_YUV420P ||
    ...
    aptima::buf_t locked_buf = aptima_video_frame->lock_buf();
...
bool demuxer_t::decode_next_video_packet_(DECODE_STATUS &decode_status) {
  ...
  if (ffmpeg_rc == AVERROR(EAGAIN)) {
    ...
    if (video_frame != nullptr) {
      ...
      aptima_env_proxy_->notify([video_frame_shared](aptima::aptima_env_t &aptima_env) {
  ...
  while (true) {
    ...
    if (ffmpeg_rc == AVERROR(EAGAIN)) {
      ...
      if (audio_frame != nullptr) {
        ...
        aptima_env_proxy_->notify([audio_frame_shared](aptima::aptima_env_t &aptima_env) {
