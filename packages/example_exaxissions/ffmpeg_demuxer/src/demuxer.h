...
class demuxer_t {
 public:
  demuxer_t(aptima::axis_env_proxy_t *axis_env_proxy,
 ...
 private:
  ...
  std::unique_ptr<aptima::audio_frame_t> to_axis_audio_frame_(const AVFrame *frame);
  std::unique_ptr<aptima::video_frame_t> to_axis_video_frame_(const AVFrame *frame);
  ...
  aptima::axis_env_proxy_t *axis_env_proxy_;
